...
test("Ghost Club requires explicit consent, reports safely and clears local identity after deletion",async({page})=>{
  ...
  await page.route("**/v1/**",async route=>{
    ...
    if(request.url().endsWith("/report")){
      await route.fulfill({status:201,contentType:"application/json",body:JSON.stringify({ok:true,report_id:"R-TEST"})});
      return;
    }
  ...
  const report=await page.evaluate(()=>(globalThis as any).GhostClubs.reportGhost("G-REPORT123","trademark"));
  ...
  expect(Object.values(local)).toEqual(Array(6).fill(null));
