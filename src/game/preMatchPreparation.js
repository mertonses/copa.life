...
(function(root){
  ...
  function render(){
    ...
    if(resource){resource.dataset.pointsLeft=String(remaining);resource.querySelectorAll("[data-prep-point]").forEach((point,index)=>point.classList.toggle("is-available",index<remaining));}
  ...
  function open(round,opponent){
    ...
    root.showModal(`<div class="prep-modal"><header><span>${tr()?"MAÇ ÖNCESİ":"PRE-MATCH"}</span><div class="prep-title-row"><div><h3>${tr()?"Hazırlık tahtası":"Preparation board"}</h3><p>${opponent&&opponent.name?opponent.name:""}</p></div><button type="button" class="prep-help" onclick="CopaPreparation.openHelp()" aria-label="${tr()?"Antrenman mekaniğini açıkla":"Explain training mechanics"}">?</button></div></header><div class="prep-resource-strip" data-prep-resource aria-label="${tr()?"Hazırlık puanı":"Preparation points"}"><div class="prep-resource-main"><small>${tr()?"HAZIRLIK PUANI":"PREPARATION POINTS"}</small><strong><b data-prep-points>2</b><span>/2</span></strong></div><div class="prep-resource-dots" aria-hidden="true"><i data-prep-point></i><i data-prep-point></i></div><em>${tr()?"Bu tur kullanılabilir":"Available this round"}</em></div><button type="button" class="prep-recommended-action" onclick="CopaPreparation.applyRecommended()"><span class="prep-recommend-copy"><small>${tr()?"ÖNERİLEN PLAN":"RECOMMENDED PLAN"}</small><b>${tr()?"Planı uygula":"Apply plan"}</b><em>${tr()?"2 PUAN · RAKİBE GÖRE":"2 POINTS · MATCHED TO OPPONENT"}</em></span><i aria-hidden="true">→</i></button>${warning?`<div class="prep-plan-warning" role="status"><b>${tr()?"PLAN UYARISI":"PLAN WARNING"}</b><span>${warning}</span></div>`:""}<div class="prep-status" data-prep-status></div><div class="prep-grid">${cards}</div><div class="bact"><button class="btn btn-primary" onclick="closeModal();renderHub()">${tr()?"PLANI UYGULA":"APPLY PLAN"}</button></div></div>`,{dismissOnOverlay:true,label:tr()?"Maç öncesi hazırlık":"Pre-match preparation",sheetClass:"sheet-preparation"});
    render();
  }
