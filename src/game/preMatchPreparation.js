...
(function(root){
  ...
  function open(round,opponent){
    ...
    root.showModal(`<div class="prep-modal"><header><span>${tr()?"MAÇ ÖNCESİ":"PRE-MATCH"}</span><div class="prep-title-row"><div><h3>${tr()?"Hazırlık tahtası":"Preparation board"}</h3><p>${opponent&&opponent.name?opponent.name:""}</p></div><button type="button" class="prep-help" onclick="CopaPreparation.openHelp()" aria-label="${tr()?"Antrenman mekaniğini açıkla":"Explain training mechanics"}">?</button></div></header><button type="button" class="prep-recommended-action" onclick="CopaPreparation.applyRecommended()"><span><small>${tr()?"TEK DOKUNUŞ":"ONE TAP"}</small><b>${tr()?"Önerilen planı uygula":"Apply recommended plan"}</b></span><i aria-hidden="true">→</i></button>${warning?`<div class="prep-plan-warning" role="status"><b>${tr()?"PLAN UYARISI":"PLAN WARNING"}</b><span>${warning}</span></div>`:""}<div class="prep-status" data-prep-status></div><div class="prep-grid">${cards}</div><div class="bact"><button class="btn btn-primary" onclick="closeModal();renderHub()">${tr()?"PLANI UYGULA":"APPLY PLAN"}</button></div></div>`,{dismissOnOverlay:true,label:tr()?"Maç öncesi hazırlık":"Pre-match preparation",sheetClass:"sheet-preparation"});
    render();
  }
