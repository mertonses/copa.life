...
function draftOptions(pos){const SG=groupOf(pos);let inp=availG(SG).slice(),oth=availAll().filter(p=>p[2]!==SG&&p[2]!=="GK"&&SG!=="GK").slice();const out=[];
  function takeFrom(a){if(!a.length)return null;let j=rand()*a.length|0;if(typeof chairman!=="undefined"&&chairman&&chairman.id==="leydi"&&rand()<0.20){const locals=[];a.forEach((p,i)=>{if(p&&p[5])locals.push(i);});if(locals.length)j=rnd(locals);}const p=a.splice(j,1)[0];return mkOpt(p[0],p[1],p[2],pos,false,p[3],p[4],p[5]);}
