...
const TRAITS={hizli:{e:"⚡",pw:()=>(style==="kontra"||style==="gegen")?2:0},lider:{e:"🧭",pw:()=>round>=6?2:0},buyukmac:{e:"🎯",pw:()=>round>=5?2:0},sorunlu:{e:"💢",pw:()=>0},cam:{e:"🩹",pw:()=>0},wonderkid:{e:"🌟",pw:()=>0}};
...
function mkOpt(name,ov,nat,pos,fab,club,age,tr){const pr=valueOf(ov);const o={name,ov,natG:nat,pos,club:club||"",age:age||26,tr:tr||0,price:(!fab&&typeof deadlineH!=="undefined"&&deadlineH<=6)?Math.max(1,Math.round(pr*1.15)):pr,fab:!!fab,train:0,dev:0,trait:null,eff:ov-misPen(nat,groupOf(pos))};assignTrait(o);return o;}
...
function discountRangeFor(ov){if(ov>=85)return [25,45];if(ov>=80)return [20,40];if(ov>=75)return [20,40];if(ov>=70)return [15,30];return [10,22];}
function applyBargain(o){if(!o||o.hidden||o.free||o.bargain)return;const rg=discountRangeFor(o.ov),id=chairman&&chairman.id,bonus=(id==="pinti")?8:0,pct=Math.min(55,ri(rg[0]+bonus,rg[1]+bonus));o.oldPrice=o.price;o.discountPct=pct;o.price=Math.max(1,Math.round(o.oldPrice*(100-pct)/100));o.bargain=true;if(!o.trait&&rand()<0.25)o.trait=rnd(["cam","sorunlu"]);}
function makeHidden(o){const r=rand(),base=o.ov;o.hidden=true;o.trait=null;o.price=5;o.oldPrice=0;o.discountPct=0;if(r<0.45){o.hiddenTier="gem";o.ov=Math.min(90,Math.max(base+ri(5,12),76));o.scoutHint=LANG==="tr"?"Scout notu: yaşına göre fazla iyi görünüyor.":"Scout note: looks too good for his age.";}else if(r<0.80){o.hiddenTier="normal";o.ov=Math.max(60,Math.min(82,base+ri(-2,3)));o.scoutHint=LANG==="tr"?"Scout notu: temiz ama sıradan rapor.":"Scout note: clean but ordinary report.";}else{o.hiddenTier="bust";o.ov=Math.max(55,base-ri(6,12));o.scoutHint=LANG==="tr"?"Scout notu: menajer fazla övüyor.":"Scout note: agent is overselling him.";}o.eff=o.ov-misPen(o.natG,groupOf(o.pos));}
function draftOptions(pos){const SG=groupOf(pos);let inp=availG(SG).slice(),oth=availAll().filter(p=>p[2]!==SG).slice();const out=[];
  function takeFrom(a){if(!a.length)return null;const j=rand()*a.length|0,p=a.splice(j,1)[0];return mkOpt(p[0],p[1],p[2],pos,false,p[3],p[4],p[5]);}
