...
var CARDDEFS={
 ...
 taraftar:{price:4,kind:"power",mode:"contract",
   eff:(s,r)=>{const common=r<=1?2:r===2?3:4;return variantOf("taraftar")===1?(r<=1?3:r===2?5:6):common;}},
 genc:{price:6,kind:"power",mode:"scaling",
   eff:(s,r)=>{const base=Math.min(6,r);return variantOf("genc")===1?Math.min(9,base+3):base;}},
 ch_momentum:{price:4,darkPrice:6,kind:"power",mode:"scaling",
   eff:(s,r)=>{const v=variantOf("ch_momentum")===1;return r<=2?(v?2:1):r<=4?(v?5:3):(v?8:5);}},
 ...
 tecrubeli_omurga:{price:5,darkPrice:6,kind:"squad",mode:"scaling",
 ...
 yerli_blok:{price:6,kind:"squad",mode:"scaling",
   eff:(s,r)=>{const n=s.filter(p=>p&&p.tr).length,v=variantOf("yerli_blok")===1;return Math.min(v?8:5,n*(v?2:1));}},
...
kanat_akini:{price:6,kind:"power",mode:"scaling",
 ...
 derbi:{price:10,kind:"final",mode:"scaling",
   eff:(s,r)=>{const v=variantOf("derbi");const g=v===1?[4,7,6]:[2,4,8];return r>=7?g[2]:r>=6?g[1]:r>=5?g[0]:0;}},
 final_provasi:{price:6,kind:"final",mode:"scaling",
   eff:(s,r)=>r>=7?(variantOf("final_provasi")===1?9:5):0},
 son_dans:{price:6,kind:"final",mode:"scaling",
   eff:(s,r)=>{if(r<7)return 0;const v=variantOf("son_dans");const healthy=!(s||[]).some(p=>p&&p.injured);return v===1?(healthy?10:0):(healthy?7:3);}},
 /* EKONOMİ */
 taksit_transfer:{price:0,kind:"economy",mode:"instant",eff:(s,r)=>0},
 son_kredi:{price:0,kind:"economy",mode:"instant",eff:(s,r)=>0},
 /* RİSK */
 kara_borsa:{price:3,darkPrice:7,kind:"risk",mode:"instant",eff:(s,r)=>0},
 sahte_evrak:{price:6,darkPrice:8,kind:"risk",mode:"contract",
   eff:(s,r)=>variantOf("sahte_evrak")===1?8:5},
 deplasman_kafilesi:{price:4,darkPrice:6,kind:"risk",mode:"instant",eff:(s,r)=>0},
 ...
 doping:{price:7,darkPrice:9,kind:"risk",mode:"contract",
   eff:(s,r)=>variantOf("doping")===1?9:6},
 kriz:{price:9,darkPrice:12,kind:"final",mode:"instant",eff:(s,r)=>0},
 kurban_belli:{price:6,kind:"risk",mode:"instant",eff:(s,r)=>0},
 primler_yatinca:{price:0,fixedPrice:true,kind:"power",mode:"instant",eff:(s,r)=>0},
 vur_igneyi:{price:3,fixedPrice:true,kind:"risk",mode:"instant",eff:(s,r)=>0},
 bu_adam:{price:6,kind:"power",mode:"instant",eff:(s,r)=>0},
  gec_gec:{price:5,kind:"defense",mode:"scaling",
   eff:(s,r)=>{const ownPower=typeof squadPower==="function"?squadPower(r,"gec_gec").power:(s.length?Math.round(s.reduce((a,p)=>a+effOf(p),0)/s.length):0);const stronger=typeof opponent!=="undefined"&&opponent&&opponent.power>ownPower;const base=stronger?5:2;return variantOf("gec_gec")===1?base+2:base;}},
 nasip_kismet:{price:2,kind:"risk",mode:"instant",eff:(s,r)=>0},
