...
var CARDDEFS={
 ...
 buyuk_mac:{price:6,kind:"power",mode:"instant",
   eff:(s,r)=>0},
 yildiz:{price:6,kind:"power",mode:"instant",
   eff:(s,r)=>0},
 /* SAVUNMA */
 ...
 tecrubeli_omurga:{price:6,kind:"squad",mode:"scaling",
   eff:(s,r)=>s.filter(p=>p&&p.age>=32).length*(variantOf("tecrubeli_omurga")===1?4:2)},
 yerli_blok:{price:8,kind:"squad",mode:"scaling",
   eff:(s,r)=>s.filter(p=>p&&p.tr).length*(variantOf("yerli_blok")===1?3:1)},
 /* HÜCUM */
 kanat_akini:{price:7,kind:"power",mode:"scaling",
   eff:(s,r)=>s.filter(p=>p&&["LW","RW","LM","RM","WB"].includes(p.pos)).length*(variantOf("kanat_akini")===1?4:2)},
 cift_forvet:{price:6,kind:"power",mode:"scaling",
   eff:(s,r)=>s.filter(p=>p&&p.pos==="ST").length*(variantOf("cift_forvet")===1?4:2)},
 /* FİNAL */
 ...
 doping:{price:0,kind:"risk",mode:"contract",
 ...
 primler_yatinca:{price:0,kind:"power",mode:"instant",eff:(s,r)=>0},
