...
function simulateMatch(myPow, oppPow, rng) {
  ...
  while(!gameOver){
    ...
    else if(roll<0.74){const fouler=side==="A"?_pickerB(rng):_pickerA(rng);const isYellow=rng()<0.6;const comm=isYellow?"🟨 "+fouler+" — "+(isTR?"Sarı kart!":"Yellow card!"):"⚠️ "+fouler+" — "+(isTR?"Faul!":"Foul!");events.push({minute:Math.floor(clock),type:isYellow?"yellow":"foul",side:side==="A"?"B":"A",pos:evPos(side,"chance"),label:isYellow?(isTR?"Sarı kart":"Yellow card"):(isTR?"Faul":"Foul"),comm});updateMom(side==="A"?-2:2);}
