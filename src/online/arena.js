(function(root){
  ...
  const COPY={
    tr:{
      ...
      win:"GALİBİYET",loss:"MAĞLUBİYET",draw:"BERABERLİK",searchAgain:"YENİ EŞLEŞME",
    ...
    en:{
      ...
      win:"VICTORY",loss:"DEFEAT",draw:"DRAW",searchAgain:"NEW MATCH",
  ...
  function chrome(content,kicker="COPA ARENA"){
    return `<div class="arena-backdrop" aria-hidden="true"><i></i><i></i><i></i></div><header class="arena-topbar"><button type="button" data-arena-action="close" aria-label="${esc(text("back"))}">←</button><div><span>${esc(kicker)}</span><b>COPA <em>ARENA</em></b></div><div class="arena-live-mark"><i></i> LIVE</div></header><div class="arena-content">${content}</div>`;
  }
  function terms(){
  ...
  function live(game){
    const events=(game.events||[]).slice(-5).reverse(),score=game.score||[0,0];
  ...
  function connectRoom(saved){
    ...
    socket.addEventListener("message",event=>{
      ...
      if(data.type==="state"){
        ...
        if(data.state.phase==="result"){remove(ROOM_KEY);telemetry("arena_match_completed",data.state.result.outcomes[data.state.selfIndex]);}
      }
    });
