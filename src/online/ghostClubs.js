...
(function(){
  ...
  function normalizeCompletedRun(context){
    ...
    const tournamentSnapshot=tournamentState&&tournamentState.format==="groups32_v2"?{format:"groups32_v2",stage:tournamentState.player&&tournamentState.player.exitStage||tournamentState.knockout&&tournamentState.knockout.round||tournamentState.phase||"group",group:{id:playerGroup&&playerGroup.id||"",rank:Math.round(Number(playerRow&&playerRow.rank)||0),points:Math.round(Number(playerRow&&playerRow.points)||0),gd:Math.round(Number(playerRow&&playerRow.gd)||0),qualified:!!(tournamentState.group&&tournamentState.group.qualified)}}:null;
    const profile={
      ...
      match_history:(Array.isArray(context.fixtures)?context.fixtures:[]).slice(0,7).map((match,index)=>({opponent:cleanText(match.opp||match.name||""),result:cleanText(match.res||""),gf:Math.round(bounded(match.gf,0,20)),ga:Math.round(bounded(match.ga,0,20)),stage:index<3?"group":index===3?"roundof16":index===4?"quarterfinal":index===5?"semifinal":"final",penalty:Array.isArray(match.penalty)?match.penalty.slice(0,2).map(value=>Math.round(bounded(value,0,20))):null})),
      reached_round:Math.round(bounded(result.round||context.round,1,7)),
  ...
  function normalizeLeaderboardRun(context){
    ...
    const fixtures=(Array.isArray(safe.fixtures)?safe.fixtures:[]).slice(0,7).map((match,index)=>({
      ...
      stage:index<3?"group":index===3?"roundof16":index===4?"quarterfinal":index===5?"semifinal":"final",
      penalty:Array.isArray(match&&match.penalty)?match.penalty.slice(0,2).map(value=>Math.round(bounded(value,0,20))):null
    }));
    while(fixtures.length<7){const index=fixtures.length;fixtures.push({opponent:"",result:"",gf:0,ga:0,stage:index<3?"group":index===3?"roundof16":index===4?"quarterfinal":index===5?"semifinal":"final",penalty:null});}
    ...
    return {
      ...
      reached_round:Math.round(bounded(result.won?7:(result.round||safe.round),1,7)),
      result:{won:!!result.won,score:cleanText(result.score||""),end_type:cleanText(result.endType||"")},
  ...
  async function renderLeaderboard(root){
    ...
    try{
      ...
      const nearby=profile&&Array.isArray(mine.nearby)?mine.nearby:[];
      const nearbyIds=new Set(nearby.map(row=>row&&row.public_club_id).filter(Boolean));
      const topClubs=(Array.isArray(top.clubs)?top.clubs:[]).filter(row=>!nearbyIds.has(row&&row.public_club_id));
      root.innerHTML=`<div class="world-season-head"><span><small>${tr("AKTİF SEZON","ACTIVE SEASON")}</small><b>${escapeHTML(top.season||new Date().toISOString().slice(0,7))}</b></span><span class="${profile&&profile.rank?"is-ranked":""}"><small>${tr("DÜNYA SIRAN","WORLD RANK")}</small><b>${profile&&profile.rank?`#${profile.rank} · ${profile.season_score}`:joined?tr("İlk turunu tamamla","Complete your first run"):tr("Henüz katılmadın","Not joined yet")}</b></span></div>
        ...
        ${joined&&!profile?`<div class="world-new-card"><span aria-hidden="true">◎</span><p><b>${tr("Sıralama profilin hazır","Your ranking profile is ready")}</b><small>${tr("İlk dereceli turunu tamamladığında burada yerini göreceksin.","Complete your first ranked run to see your place here.")}</small></p></div>`:""}
        ${nearby.length?`<section class="world-nearby-section"><div class="meta-section-heading"><h4>${tr("SIRALAMADAKİ YERİN","YOUR POSITION")}</h4><small>${tr("Kulübün ve yakınları","Your club and nearby clubs")}</small></div>${leaderboardRows(nearby,profile&&profile.public_club_id)}</section>`:""}
        <aside class="copa-list-ad-slot" id="worldRankingAdSlot" aria-label="${tr("Reklam alanı","Advertisement area")}"></aside>
        <section class="world-top-section"><div class="meta-section-heading"><h4>${tr("DÜNYA SIRALAMASI","WORLD RANKING")}</h4><small>${tr("İLK 100 · BETA","TOP 100 · BETA")} · ${escapeHTML(top.season||"")}</small></div><div class="world-rank-list">${leaderboardRows(topClubs,profile&&profile.public_club_id)||`<div class="meta-inline-empty">${tr("Bu sezon henüz sıralanan kulüp yok.","No clubs are ranked this season yet.")}</div>`}</div></section>
        <details class="world-rule-disclosure"><summary>${tr("SIRALAMA KURALI","RANKING RULE")}<span aria-hidden="true">⌄</span></summary><p class="world-rule">${tr("Kulüp Katsayısı bu ay kabul edilen en iyi 10 turun Dünya İtibarı toplamıdır; eşit Katsayılar aynı sırayı paylaşır. Kalıcı Dünya İtibarı sezon sonunda sıfırlanmaz. Bu ödülsüz topluluk sıralamasında tutarlılık kontrolleri vardır, mutlak hile koruması yoktur. Dünya profili cihaz anahtarına bağlıdır ve kariyer kayıt koduyla taşınmaz.","Club Coefficient is the World Reputation total of your best 10 accepted runs this month; equal Coefficients share the same rank. Permanent World Reputation never resets. This reward-free community ranking uses consistency checks, not absolute cheat protection. The World profile is device-bound and does not transfer with the career save code.")}</p></details>`;
  ...
  async function findOpponent(criteria){
    ...
    const round=Math.round(bounded(criteria&&criteria.round,1,7));
  ...
  function requestMobileOnboarding(){
    ...
    onboardingRequest=new Promise(resolve=>{
      ...
      layer.innerHTML=`<section class="ghost-consent-card online-onboarding-card" role="dialog" aria-modal="true" aria-labelledby="onlineOnboardingTitle"><div class="online-onboarding-kicker">${tr("İLK KURULUM · ÇEVRİMİÇİ OYUN","FIRST SETUP · ONLINE PLAY")}</div><h3 id="onlineOnboardingTitle">${tr("Çevrimiçi özelliklerini seç","Choose online features")}</h3><p>${tr("Seçimlerini daha sonra Gelişmiş Ayarlar’dan değiştirebilirsin.","You can change these later in Advanced Settings.")}</p><div class="online-onboarding-options"><label><input type="checkbox" data-online-matching ${enabled()?"checked":""}><span><b>${tr("Ghost rakipler","Ghost opponents")}</b><small>${tr("Anonim rakip kadroları indirir. Kulübünü paylaşmaz.","Downloads anonymous opponents. Does not share your club.")}</small></span></label><label><input type="checkbox" data-online-sharing ${sharingEnabled()?"checked":""}><span><b>${tr("Kulübümü paylaş","Share my club")}</b><small>${tr("Tamamlanan kadron 45 gün Ghost havuzunda kalır.","Your completed squad stays in the Ghost pool for 45 days.")}</small></span></label><label><input type="checkbox" data-online-leaderboard ${leaderboardEnabled()?"checked":""}><span><b>${tr("Dünya sıralaması","World ranking")}</b><small>${tr("Kulüp ve kupa istatistiklerin herkese açık olur.","Your club and cup statistics become public.")}</small></span></label></div><p class="ghost-consent-confirm">${tr("TÜMÜNÜ AÇ VE KABUL ET ile şartları ve seçili paylaşımları onaylarsın.","ENABLE ALL AND ACCEPT confirms the Terms and selected sharing.")}</p><div class="ghost-consent-links"><a href="terms.html" target="_blank" rel="noopener">${tr("Kullanım şartları","Terms")}</a><a href="privacy.html" target="_blank" rel="noopener">${tr("Gizlilik","Privacy")}</a></div><div class="online-onboarding-actions"><button type="button" data-online-offline>${tr("ÇEVRİMDIŞI DEVAM","CONTINUE OFFLINE")}</button><button type="button" data-online-save>${tr("SEÇİMLERİ KAYDET","SAVE CHOICES")}</button><button type="button" data-online-all>${tr("TÜMÜNÜ AÇ VE KABUL ET","ENABLE ALL AND ACCEPT")}</button></div></section>`;
  ...
  function ensureSetting(){
    ...
    if(copy)copy.textContent=on?tr("Bu tur uygun bir Ghost rakiple eşleşilebilir.","This run can match an eligible Ghost opponent."):tr("Bu tur uygun bir Ghost rakiple eşleşemez.","This run cannot match an eligible Ghost opponent.");
    ...
    if(shareCopy)shareCopy.textContent=sharing?tr("Kulübün tur sonunda Ghost havuzuna katılır.","Your club joins the Ghost pool at the end of the run."):tr("Kulübün tur sonunda Ghost havuzuna katılmaz.","Your club does not join the Ghost pool at the end of the run.");
