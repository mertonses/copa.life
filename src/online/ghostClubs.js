...
(function(){
  ...
  function requestConsent(){
    ...
    document.body.appendChild(layer);const terms=layer.querySelector("[data-ghost-terms]"),sharing=layer.querySelector("[data-ghost-sharing]"),accept=layer.querySelector("[data-ghost-accept]"),refresh=()=>{accept.disabled=!(terms.checked&&sharing.checked);};terms.addEventListener("change",refresh);sharing.addEventListener("change",refresh);layer.querySelector("[data-ghost-cancel]").addEventListener("click",()=>complete(false));accept.addEventListener("click",()=>{safeSet(CONSENT_KEY,JSON.stringify({version:CONFIG.consentVersion,terms:true,sharing:true,accepted_at:new Date().toISOString()}));safeSet(SHARING_KEY,"1");if(window.CopaAnalytics)window.CopaAnalytics.track("ghost_opt_in");deleteToken();ensureSetting();flushQueue();complete(true);});
