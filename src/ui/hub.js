...
function prepareChairmanRoundEvent(forceType){
  ...
  let type=forceType||"";
  if(!type&&chairman.id==="sansasyoncu"&&round<7&&round%2===1)type="spotlight";
  else if(!type&&chairman.id==="torpilci"&&round===3&&!eventSeen.nephewDecision)type="nephew";
  else if(!type&&chairman.id==="cilgin"&&round>=2&&round<7)type="chaos";
  if(!type)return null;
...
function canOfferRewardedMarketReroll(){return !!(window.COPA_IS_NATIVE&&window.COPA_PLATFORM==="android"&&window.CopaNativeAds&&typeof window.CopaNativeAds.showRewardedMarket==="function"&&rewardedMarketRerollsEarned<MAX_REWARDED_MARKET_REROLLS);}
async function requestRewardedMarketReroll(){if(shopRewardInFlight||!canOfferRewardedMarketReroll())return;shopRewardInFlight=true;renderHub();let result={earned:false,reason:"native_error"};try{result=await window.CopaNativeAds.showRewardedMarket(chairmanEventRunId);}catch(_){ }shopRewardInFlight=false;if(result&&result.earned){rewardedMarketRerollsEarned=Math.min(MAX_REWARDED_MARKET_REROLLS,Math.max(rewardedMarketRerollsEarned+1,Number(result.earnedCount)||0));newShopOffers();_saveState();showToast(LT("Kart pazarı ücretsiz yenilendi.","Card market refreshed for free.","Mercado de cartas renovado gratis.","Kartenmarkt gratis aktualisiert.","Mercato carte aggiornato gratis."),{type:"info"});sfxTick();renderHub();return;}showToast(result&&result.reason==="not_ready"?LT("Reklam henüz hazır değil; biraz sonra tekrar dene.","The ad is not ready; try again shortly.","El anuncio aún no está listo; inténtalo pronto.","Die Anzeige ist noch nicht bereit; versuche es gleich erneut.","L'annuncio non è pronto; riprova tra poco."):LT("Reklam tamamlanmadı; pazar yenilenmedi.","The ad was not completed; market unchanged.","El anuncio no se completó; mercado sin cambios.","Die Anzeige wurde nicht abgeschlossen; Markt unverändert.","L'annuncio non è stato completato; mercato invariato."));renderHub();}
