...
function renderHub(){const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  $("playBtn").textContent=round>=6?x.playFinal:x.play;$("autoLbl").textContent=autoPlay?x.autoOn:x.autoOff;$("autoTog").classList.toggle("on",autoPlay);
  $("talkBtn").classList.toggle("hidden",talkUsed);$("talkBtn").textContent=x.talk;renderDebtWarning();renderInjbar();
