...
function _breakdownModal(title,meta,rows,{label=""}={}){
  const close=LANG==="tr"?"Kapat":"Close";
...
function shopCardDesc(k,raw,variantOverride){
  ...
  const specific={
    ...
    altyapi_plani:()=>sv===1?(tr?"İlk 11'deki 23 yaş ve altı oyuncu başına +2 güç, max +6. Finalde -4.":"Age-23-or-under starter +2 each, max +6. -4 in the final."):(tr?"İlk 11'deki 23 yaş ve altı oyuncu başına +1 güç, max +4.":"Age-23-or-under starter +1 each, max +4."),
    yerli_blok:()=>sv===1?(tr?"İlk 11'deki yerli başına +2 güç, max +5. Finalde -3.":"Local starter +2 each, max +5. -3 in the final."):(tr?"İlk 11'deki yerli başına +1 güç, max +5.":"Local starter +1 each, max +5."),
    ...
    otobus:()=>sv===1?(tr?"Stoper başına +4 güç, en fazla +12. %10 ihtimal -€6M ceza; finalde -3.":"CB +4 each, max +12. 10% chance -€6M fine; -3 in the final."):(tr?"Stoper başına +3 güç, en fazla +9.":"CB +3 each, max +9."),
    ...
    son_kredi:()=>sv===1?(tr?"Kasa -€10M altındaysa +€20M; değilse bekler. Güven -1 ve kovulma eşiği €5M daralır.":"Below -€10M: +€20M; otherwise waits. Trust -1 and the sack limit tightens by €5M."):(tr?"Kasa -€10M altındaysa +€15M; değilse bekler. Kovulma eşiği €5M daralır.":"Below -€10M: +€15M; otherwise waits. The sack limit tightens by €5M."),
    ...
    kriz:()=>sv===1?(tr?"Final cezasının yarısını, en fazla 8 gücü telafi eder. %20 ihtimalle €5M ek masraf.":"Offsets half the final penalty, up to 8 power. 20% chance of an extra €5M cost."):(tr?"Final cezasının yarısını, en fazla 6 gücü telafi eder.":"Offsets half the final penalty, up to 6 power."),
...
function cardContractText(k){
  ...
  const map={
    ...
    derbi:round<4?(tr?"İlk 3 tur etkisiz; çeyrek finalde açılır.":"Inactive for the first 3 rounds; activates in the quarter-final."):(variantOf(k)===1?(tr?"Büyük maç bonusu bu tur "+plus+"; finalde -4 güç.":"Big-match bonus is "+plus+" this round; -4 power in the final."):(tr?"Büyük maç bonusu bu tur "+plus+".":"Big-match bonus is "+plus+" this round.")),
    ...
    tecrubeli_omurga:variantOf(k)===1?(tr?"İlk 11'deki 32+ oyuncu başına +2, en fazla +6; şu an "+plus+".":"Each age-32+ starter adds +2, max +6; now "+plus+"."):(tr?"İlk 11'deki 32+ oyuncu başına +1, en fazla +4; şu an "+plus+".":"Each age-32+ starter adds +1, max +4; now "+plus+"."),
    ...
    kriz:variantOf(k)===1?(tr?"Final cezasının yarısını, en fazla 8 gücü telafi eder.":"Offsets half the final penalty, up to 8 power."):(tr?"Final cezasının yarısını, en fazla 6 gücü telafi eder.":"Offsets half the final penalty, up to 6 power."),
...
function renderHub(){try{if(typeof _saveState==="function")_saveState();}catch(e){}const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  {const cb=chemBonus(s);const cvClamped=Math.max(-3,Math.min(5,cb.total));$("chemV").textContent=(cvClamped>=0?"+":"")+cvClamped;$("chemList").innerHTML=x.powHint;const hp=$("hubPitch");if(hp){hp.classList.toggle("chem-high",cvClamped>=3);hp.classList.toggle("chem-low",cvClamped<0);}
