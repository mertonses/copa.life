...
function renderHub(){if(typeof _currentCaptainPlayer==="function")_currentCaptainPlayer();try{if(typeof _saveState==="function")_saveState();}catch(e){}const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  cards.forEach(k=>{const d=document.createElement("div");const v=variantOf(k);d.className="card v"+v+" cat-"+(CATMAP[k]||"gorev");const cd=x.cards[k],cv=cardVisiblePower(k,s,round),stance=variantBadge(v);d.innerHTML=`<span class="card-ico">${CARD_SVGS[k]||cd.i}</span><b>${cd.n}</b><span class="tier">${kindLabel(k)}</span> <span class="v">${cv>=0?"+":""}${cv}</span><span class="copy active-stance stance-${v}">${stance}</span>`;cr.appendChild(d);});
  ...
  {const fp=$("finalPills");if(fp){const active=cards.filter(k=>cardVisiblePower(k,s,round)!==0);if(active.length){fp.innerHTML=active.slice(0,5).map(k=>{const cd=x.cards[k],v=cardVisiblePower(k,s,round),cls=v>0?"fp-bonus":"fp-risk";return `<span class="fpill ${cls}" onclick="showCardPopup('${k}')"><span class="fpico">${CARD_SVGS[k]||cd.i}</span> ${cd.n} <b>${v>=0?"+":""}${v}</b></span>`;}).join("")+(active.length>5?`<span class="fpill fp-none">+${active.length-5}</span>`:"");}else fp.innerHTML="";}}
  ...
  shopOffers.slice(0,3).forEach(k=>{
    ...
    d.innerHTML=`<div class="ct-head">${_darkBadge}<span class="ct-price ct-head-price ${pr<=0?"ct-price-free":""}">${priceLabel}</span></div><div class="ct-body"><div class="ct-titlegroup"><span class="ct-art" aria-hidden="true">${CARD_SVGS[k]||cd.i}</span><div class="ct-name">${cd.n}</div></div><div class="market-card-impact"><span>${_impactScope} <b class="${_powerDelta>=0?"is-positive":"is-negative"}">${_powerDelta>=0?"+":""}${_powerDelta}</b></span></div><div class="ct-desc">${desc}</div></div><div class="ct-foot"><span class="ct-contract">${cardContractType(k)}${_darkPenBadge}</span><button type="button" class="ct-detail" onclick="event.stopPropagation();if(window.CopaAnalytics)CopaAnalytics.track('card_effect_summary_viewed',{detail:'market',card_kind:'${analyticsCardKind(k)}'});CopaMobileShell.openMarketCard(this)">${LT("ETKİ ÖZETİ","EFFECT SUMMARY","RESUMEN","EFFEKTE","RIEPILOGO")}</button></div><div class="ct-detail-data" hidden>${_effectSummary}${_syn?`<div class="synergy-preview">${LANG==="tr"?_syn.tr:_syn.en}</div>`:""}</div><div class="insufficient-pop" aria-hidden="true">${LANG==="tr"?"Kasa yetersiz":"Insufficient funds"}</div>`;
