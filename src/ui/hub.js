...
function _applyGhostOpponent(ghost,expectedRound,baseline,options={}){
  ...
  const tournamentMode=!!(window.CopaTournamentRuntime&&window.CopaTournamentRuntime.active());
  if(tournamentMode&&!window.CopaTournamentRuntime.replaceCurrentOpponent(ghost))return;
  opponent=tournamentMode?window.opponent:ghost;
  if(!tournamentMode&&Array.isArray(bracket))bracket[round-1]=ghost;
...
function _lockGhostOpponent(){
  if(window.CopaTournamentRuntime&&window.CopaTournamentRuntime.active()&&!['quarterfinal','semifinal'].includes(window.CopaTournamentRuntime.stage()))return false;
...
function prepareChairmanRoundEvent(forceType){
  ...
  let type=forceType||"";
  if(!type&&chairman.id==="sansasyoncu"&&(round===1||round===4))type="spotlight";
  else if(!type&&chairman.id==="torpilci"&&round===3&&!eventSeen.nephewDecision)type="nephew";
  else if(!type&&chairman.id==="cilgin"&&round>=2&&round<7&&professorChaosOffers<2)type="chaos";
  if(!type)return null;
  ...
  if(type==="chaos")professorChaosOffers++;
...
function enterHub(restoring=false,ghostLocked=false){if(window._wantFinal){window._wantFinal=false;round=7;opponent=bracket[round-1];window.CopaRunState&&window.CopaRunState.transition("hub",{force:true,reason:"cheat_final"});setTimeout(()=>playMatch(true),300);return;}if(window._wantSeedResult&&typeof _runSeedResultCheat==="function"){const kind=window._wantSeedResult;window._wantSeedResult="";_runSeedResultCheat(kind);return;}if(window.CopaRunState&&!ghostLocked){const moved=window.CopaRunState.transition("hub",{reason:restoring?"restore":"draft_or_reward_complete"});if(!moved.ok){window.CopaDiagnostics&&window.CopaDiagnostics.capture("state_guard",moved.errors.join(","),"");return;}}clearTimeout(autoTimer);if($("intro"))$("intro").classList.add("hidden");$("ddbanner").classList.add("hidden");$("draft").classList.add("hidden");const _draw=$("tournamentDraw");if(_draw)_draw.classList.add("hidden");document.documentElement.classList.remove("tournament-draw-open");document.body.classList.remove("tournament-draw-open");$("sim").classList.add("hidden");$("result").classList.add("hidden");$("hub").classList.remove("hidden");
  ...
  if(restoring){
    ...
    renderFixtures();renderHub();if(typeof maybeDraftEvent==="function")maybeDraftEvent();if(typeof _saveState==="function")_saveState();queuePendingChairmanEvent(120);if(window.CopaRelationships)CopaRelationships.queuePending(850);if(window.CopaClubFiles)CopaClubFiles.queueSelection(1300);if(!ghostLocked)_lockGhostOpponent();return;
  }
  ...
  if(budget<-10&&chairTrust>0&&round>1&&requestChairTrustChange(-1,"debt_pressure",60))pushFeed("📉 "+(LANG==="tr"?"Başkan güveni azaldı ("+chairTrust+"/3)":"Chairman confidence drops ("+chairTrust+"/3)"),"lose");
if(chairman.id==="leydi"&&round>1){const _chem=chemBonus(picksBySlot.filter(Boolean)).total;if(_chem>=3){riskPowerMod+=1;requestChairTrustChange(1,"diplomat_chemistry",65);pushFeed(`<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-.15em"><path d="M3 7a3 3 0 0 1 3 -3h12a3 3 0 0 1 3 3v3a9 9 0 0 1 -9 9a9 9 0 0 1 -9 -9v-3z"/><path d="M9 12l2 2l4 -4"/></svg> `+(LANG==="tr"?"Diplomat: uyumlu kadro — bu maç +1 güç":"Diplomat: cohesive squad — +1 power"),"pres");}else if(_chem<0){riskPowerMod-=1;requestChairTrustChange(-1,"diplomat_bad_chemistry",75);pushFeed(`<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-.15em"><path d="M3 7a3 3 0 0 1 3 -3h12a3 3 0 0 1 3 3v3a9 9 0 0 1 -9 9a9 9 0 0 1 -9 -9v-3z"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg> `+(LANG==="tr"?"Diplomat: dağınık kadro — bu maç -1 güç":"Diplomat: disjointed squad — -1 power"),"lose");}}
  ...
  if(window.CopaRelationships)CopaRelationships.queuePending(1450);
  if(window.CopaClubFiles)CopaClubFiles.queueSelection(1850);
  if(typeof _saveState==="function")_saveState();
...
function showPowerGraph(){
  ...
  const svg=`<svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg" style="overflow:visible">
    <polyline points="${pts(hist)}" fill="none" stroke="var(--status-success)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    <polyline points="${pts(oppH)}" fill="none" stroke="#DA3D2E" stroke-width="1.5" stroke-dasharray="4 3" stroke-linecap="round" stroke-linejoin="round"/>
    ${hist.map((v,i)=>`<circle cx="${pad+i*xS}" cy="${H-pad-v*yS}" r="3" fill="var(--status-success)"/>`).join('')}
    <text x="4" y="12" font-size="8" fill="var(--status-success)" font-family="monospace">${tr?"Sen":"You"}</text>
    <text x="4" y="22" font-size="8" fill="#DA3D2E" font-family="monospace">${tr?"Rakip":"Opp"}</text>
  </svg>`;
...
function showPowerBreak(){const x=L(),tr=LANG==="tr",sp=squadPower(round),sgn=v=>(v>=0?"+":"")+Math.round(v),waste=Math.max(0,Math.round(sp.capLoss));const rows=[_breakdownRow(x.pbPlayers,Math.round(sp.avg),{neutral:true}),...(sp.starImpact?[_breakdownRow(tr?"Yıldız etkisi":"Star impact",sgn(sp.starImpact))]:[]),_breakdownRow(x.ui.style,sgn(sp.styleBonus)),_breakdownRow(x.ui.cards,sgn(sp.cardBonus)),...(sp.loanBonus?[_breakdownRow(tr?"Kiralık güç garantisi":"Loan power guarantee",sgn(sp.loanBonus))]:[]),_breakdownRow(x.ui.matchup||(tr?"Eşleşme":"Matchup"),sgn(sp.matchup)),_breakdownRow(tr?"Risk / Final Borcu":"Risk / Final Debt",sgn(sp.risk)),_breakdownRow(tr?"\u00d6zellik + Moral":"Trait + Morale",sgn(sp.trait+sp.moral)),_breakdownRow(x.ui.bonusEfficiency,`${Math.round(sp.bonus)} / ${Math.round(sp.rawBonus)}`,{tone:"bd-value-positive"})];if(waste)rows.push(_breakdownRow(tr?"Tavan y\u00fcz\u00fcnden bo\u015fa giden bonus":"Bonus wasted by cap",`-${waste}`,{tone:"bd-value-negative"}));rows.push(_breakdownRow(x.pbChem,(sp.chem>=0?"+":"")+sp.chem,{tone:sp.chem<0?"bd-value-negative":undefined}),_breakdownRow(x.pbTotal,sp.power,{total:true,neutral:true}));_breakdownModal(x.powHdr2,x.rounds[round-1],rows,{label:x.powHdr2});}
function hiddenScoutNote(signal){if(window.CopaHiddenDraft)return CopaHiddenDraft.scoutNote(signal);const notes=LANG==="tr"?["Tavan yüksek · kesin değil","Veriler dengeli · sürpriz var","Menajer sözü veriden güçlü"]:["High ceiling · uncertain","Balanced data · surprise possible","Agent pitch exceeds data"];return notes[signal==="positive"?0:signal==="negative"?2:1];}
...
function cardStampShape(k){
  const source=String(k||""),shapeNames=["soft","slash","seal"];
  let hash=0;for(let i=0;i<source.length;i++)hash=(hash*31+source.charCodeAt(i))>>>0;
  return shapeNames[hash%shapeNames.length];
}
function cardStampAssetSlug(k){
  const key=String(k||"");
  return key&&typeof CARDDEFS!=="undefined"&&CARDDEFS[key]?key.replace(/_/g,"-"):"";
}
const MARKET_MOTION_PROFILES={
  taraftar:["fans"],genc:["hair"],ch_momentum:["sweep"],kontra:["dash"],buyuk_mac:["impact"],yildiz:["star"],
  otobus:["scan"],kaleci_kalesi:["shield"],mac_sozu:["speech"],anadolu:["route"],altyapi_plani:["rise"],
  tecrubeli_omurga:["spine"],yerli_blok:["block"],kaptanin_karari:["compass"],kanat_akini:["wing"],cift_forvet:["dual"],
...
function cardStampMarkup(k,fallback,extraClass=""){
  ...
  /* Damga dili statiktir: eski kart mikro-animasyonları küçük ölçekte titreşim ve
     görsel gürültü yaptığı için yalnızca damga ikonundan çıkarılır. */
  const icon=raw.replace(/<animate(?:Transform)?\b[^>]*\/?>/gi,"").replace(/<\/animate(?:Transform)?>/gi,"");
  const v=typeof variantOf==="function"?variantOf(k):0,kind=typeof cardKind==="function"?cardKind(k):"",isDark=v===1||/\bstamp-dark\b/.test(extraClass);
  const isSpecial=kind==="final"||kind==="risk"||kind==="temporary"||["derbi","ch_final","final_provasi","son_dans","kupaci_kadro","saha_disi_baski","riziko","buyuk_mac","doping","sahte_evrak","kriz","gec_gec","taksit_transfer"].includes(String(k||""));
  const assetSlug=cardStampAssetSlug(k),asset=assetSlug?`<img class="card-stamp-image" src="assets/cards/stamps/fullbox/${assetSlug}.webp" alt="" aria-hidden="true" draggable="false">`:icon;
  const marketMotion=extraClass.includes("stamp-tile")?marketMotionMarkup(k):"";
  return `<span class="card-stamp ${assetSlug?"card-stamp-asset card-stamp-fullbox ":""}stamp-${isDark?"dark":"base"} stamp-${cardStampShape(k)}${isSpecial?" stamp-special":""} ${extraClass}" data-card-stamp="${String(k||"")}"><span class="card-stamp-ink">${asset}</span>${marketMotion}</span>`;
}
function instantShopPowerDelta(k,variantOverride){
  ...
  const map={
    ...
    kisa_kamp:v===1?6:4,
    buyuk_mac:v===1?10:6,
    yildiz_krizi:v===1?4:3
  };
  return Object.prototype.hasOwnProperty.call(map,k)?map[k]:null;
}
CARD_SVGS.mac_sozu=`<svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M8 39V9h32v30z" opacity=".3"/><path d="M16 39V24h16v15"/><path d="M20 29h8"/><path d="M16 13c4 2 12 2 16 0"/><path d="M18 19l4 4 8-9"/><circle cx="24" cy="34" r="2.5"/></svg>`;
...
function cardVisiblePower(k,s,r){return k==="mac_sozu"&&typeof matchPromisePowerForRound==="function"?matchPromisePowerForRound(r):cardEff(k,s,r);}
...
function shopCardDesc(k,raw,variantOverride){
  ...
  const specific={
    ...
    ch_momentum:()=>sv===1?(tr?"Turnuva ritmini takım gücüne çevirir: 1-2. tur +2, 3-4. tur +5, 5. turdan itibaren +8. Finalde -6 güç.":"Converts tournament rhythm into team power: +2 in rounds 1-2, +5 in rounds 3-4, then +8. -6 power in the final."):(tr?"Turnuva ritmini takım gücüne çevirir: 1-2. tur +1, 3-4. tur +3, 5. turdan itibaren +5.":"Converts tournament rhythm into team power: +1 in rounds 1-2, +3 in rounds 3-4, then +5."),
    ...
    yerli_blok:()=>sv===1?(tr?"İlk 11'deki yerli oyuncu başına +2 güç (max +8). Finalde -3 güç.":"+2 power per local starter (max +8). -3 in the final."):(tr?"İlk 11'deki yerli oyuncu başına +1 güç (max +5).":"+1 power per local starter (max +5)."),
    ...
    deplasman_kafilesi:()=>sv===1?(tr?"Rakibin GÜÇ değeri seninkinden yüksekse +9, aksi durumda %50 +4 / %50 -2 güç.":"If the opponent's POWER is higher than yours: +9, when it is not: 50% +4 / 50% -2 power."):(tr?"Rakibin GÜÇ değeri seninkinden yüksekse +6, aksi durumda +2 güç.":"If the opponent's POWER is higher than yours: +6, when it is not: +2 power."),
    ...
    primler_yatinca:()=>sv===1?(tr?"Ücretsiz. Şimdi +9 güç. Gelecek tur −€12M.":"Free. +9 power now. Next round −€12M."):(tr?"Ücretsiz. Şimdi +5 güç. Gelecek tur −€6M.":"Free. +5 power now. Next round −€6M."),
    doping:()=>sv===1?(tr?"Her tur +9 güç; güven -1. Her tur %20 ihtimal -€18M, en fazla 2 ceza; finalde -8.":"+9 power each round; trust -1. 20% chance of an €18M fine each round, max 2 fines; -8 in the final."):(tr?"Her tur +6 güç; edinirken %20 güven riski. Her tur %25 ihtimal -€10M, en fazla 2 ceza.":"+6 power each round; 20% trust risk on acquisition. 25% chance of a €10M fine each round, max 2 fines."),
    ...
    sahte_evrak:()=>sv===1?(tr?"€8M. Her tur +8 güç; güven -1 ve finalde -8 güç.":"€8M. +8 power each round; trust -1 and -8 in the final."):(tr?"€6M. Her tur +5 güç; edinirken %18 ihtimal güven -1.":"€6M. +5 power each round; 18% chance of trust -1 on acquisition."),
    ...
    gec_gec:()=>sv===1?(tr?"Barikat eklenmeden önce rakibin GÜÇ değeri seninkinden yüksekse +7, aksi durumda +4. Her tur %25 sakatlık riski; finalde -3.":"Before Barricade is added, if the opponent's POWER is higher than yours: +7, when it is not: +4. 25% injury risk each round; -3 in the final."):(tr?"Barikat eklenmeden önce rakibin GÜÇ değeri seninkinden yüksekse +5, aksi durumda +2.":"Before Barricade is added, if the opponent's POWER is higher than yours: +5, when it is not: +2."),
    ...
    kara_borsa:()=>sv===1?(tr?"€7M; bir kart yak, 4 karttan 2'sini seç. %25 ihtimal -€8M ceza.":"€7M; burn one card, choose 2 of 4. 25% chance of an €8M fine."):(tr?"€3M; bir kart yak, 3 COMMON karttan 1'ini seç.":"€3M; burn one card, choose 1 of 3 COMMON cards."),
    nasip_kismet:()=>sv===1?(tr?"Satın alınca kupon açılır. Önündeki 2 turda alacağın ilk kart en fazla €8M ucuzlar (min. €2M). %20 ihtimalle hemen €3M masraf çıkar.":"On purchase, gain a coupon. The first card you buy within the next 2 rounds costs up to €8M less (min. €2M). 20% chance of an immediate €3M expense."):(tr?"Satın alınca kupon açılır. Önündeki 2 turda alacağın ilk kart en fazla €5M ucuzlar (min. €2M).":"On purchase, gain a coupon. The first card you buy within the next 2 rounds costs up to €5M less (min. €2M)."),
    ...
    kriz:()=>sv===1?(tr?"Final cezasının %65'ini, en fazla 10 gücü telafi eder. %20 ihtimalle €5M ek masraf.":"Offsets 65% of the final penalty, up to 10 power. 20% chance of an extra €5M cost."):(tr?"Final cezasının %50'sini, en fazla 6 gücü telafi eder.":"Offsets 50% of the final penalty, up to 6 power."),
...
function conditionHint(k){
 ...
 const map={
  yerli_blok:()=>`${tr?"Yerli":"Locals"}: ${s.filter(p=>p.tr).length}/${variantOf("yerli_blok")===1?4:5}`,
  ...
  mac_sozu:()=>typeof matchPromiseStatusText==="function"?matchPromiseStatusText():"",
  kaptanin_karari:()=>typeof captainDecisionStatusText==="function"?captainDecisionStatusText():"",
...
function cardContractText(k){
  ...
  const map={
    ...
    mac_sozu:typeof matchPromiseStatusText==="function"?matchPromiseStatusText():(tr?"Maç öncesi temiz sayfa sözü.":"Pre-match clean-sheet promise."),
    kaptanin_karari:typeof captainDecisionStatusText==="function"?captainDecisionStatusText():(tr?"Olumsuz konuşma sonrası hazır.":"Ready after a negative team talk."),
    ...
    primler_yatinca:variantOf(k)===1?(tr?"Ücretsiz; bu tur +9 güç, gelecek tur −€12M.":"Free; +9 power this round, next round −€12M."):(tr?"Ücretsiz; bu tur +5 güç, gelecek tur −€6M.":"Free; +5 power this round, next round −€6M."),
    ...
    kara_borsa:variantOf(k)===1?(tr?"Bir kart yak; 4 karttan 2'sini seç; %25 ihtimal -€8M.":"Burn one; choose 2 of 4; 25% chance -€8M."):(tr?"Bir kart yak; 3 COMMON karttan 1'ini seç.":"Burn one; choose 1 of 3 COMMON cards."),
    sahte_evrak:variantOf(k)===1?(tr?"€8M; her tur +8 güç, güven -1, finalde -8.":"€8M; +8 each round, trust -1, -8 in final."):(tr?"€6M; her tur +5 güç, edinirken %18 güven riski.":"€6M; +5 each round, 18% trust risk on acquisition."),
    ...
    yerli_blok:variantOf(k)===1?(tr?"Yerli başına +2, max +8; finalde -3; şu an "+plus+".":"Local starter +2 each, max +8, final -3; now "+plus+"."):(tr?"Yerli başına +1, max +5; şu an "+plus+".":"Local starter +1 each, max +5; now "+plus+"."),
    ...
    deplasman_kafilesi:variantOf(k)===1?(tr?"Rakip GÜÇ > senin GÜÇ: +9; aksi halde %50 +4 / %50 -2. Şu an "+plus+".":"Opponent POWER > your POWER: +9; otherwise 50% +4 / 50% -2. Now "+plus+"."):(tr?"Rakip GÜÇ > senin GÜÇ: +6; aksi halde +2. Şu an "+plus+".":"Opponent POWER > your POWER: +6; otherwise +2. Now "+plus+"."),
    ...
    son_dans:variantOf(k)===1?(tr?"Yalnızca finalde: ilk 11'de sakat yoksa +10; en az 1 sakat varsa +0 güç.":"Final only: +10 with no injured starter; +0 with at least 1."):(tr?"Yalnızca finalde: ilk 11'de sakat yoksa +7; en az 1 sakat varsa +3 güç.":"Final only: +7 with no injured starter; +3 with at least 1."),
    ...
    doping:variantOf(k)===1?(tr?"Her tur +9 güç; güven -1, her tur %20 ihtimal -€18M (max 2), finalde -8.":"+9 each round; trust -1, 20% chance of an €18M fine each round (max 2), final -8."):(tr?"Her tur +6 güç; edinirken %20 güven riski, her tur %25 ihtimal -€10M (max 2).":"+6 each round; 20% trust risk on acquisition, 25% chance of a €10M fine each round (max 2)."),
    gec_gec:variantOf(k)===1?(tr?"Barikat öncesi rakip GÜÇ > senin GÜÇ: +7; aksi halde +4. Her tur %25 sakatlık; finalde -3.":"Before Barricade, opponent POWER > your POWER: +7; otherwise +4. 25% injury each round; final -3."):(tr?"Barikat öncesi rakip GÜÇ > senin GÜÇ: +5; aksi halde +2.":"Before Barricade, opponent POWER > your POWER: +5; otherwise +2."),
    ...
    kriz:variantOf(k)===1?(tr?"Final cezasının %65'ini, en fazla 10 gücü telafi eder.":"Offsets 65% of the final penalty, up to 10."):(tr?"Final cezasının %50'sini, en fazla 6 gücü telafi eder.":"Offsets 50% of the final penalty, up to 6."),
...
function renderCollection(){const grid=$("collectionGrid");if(!grid)return;const x=L(),s=picksBySlot.filter(Boolean),cap=activeCardSlots();let owned=0,shown=0;grid.innerHTML="";renderCollectionFilters();allCardKeys().forEach(k=>{const cd=x.cards[k];if(!cd||isInstantCard(k))return;const c=invOf(k),open=c>0,active=hasCard(k),committed=active&&typeof cardHasLockedCommitment==="function"&&cardHasLockedCommitment(k),cat=CATMAP[k]||"gorev",v=variantOf(k);if(open)owned++;if(!collectionPass(k))return;shown++;const pv=open?simulateEquipPower(k):null,locked=!open;const equipLine=active?(committed?(LANG==="tr"?"✓ sözleşme aktif — çıkarılamaz":"✓ contract committed — locked"):(LANG==="tr"?"✓ aktif — çıkarmak için tıkla":"✓ active — click to remove")):(open?(cards.length<cap?(x.ui.clickEquip):(x.ui.slotFull)):(x.ui.unlockFrom));const preview=open&&!active&&cards.length<cap?`<div class="powerpreview cc-preview">${LANG==="tr"?"Takarsan":"Equip"}: ${pv.before} → ${pv.after}</div>`:"";const desc=formatCardDesc(open?(variantDesc(cd.d,v)||shortCardText(k,s)):(LANG==="tr"?"Pazardan açılır":"Unlock in market"));const d=document.createElement("div");d.className="collcard v"+v+" cat-"+cat+(locked?" locked":"")+(active?" active":"")+(committed?" committed":"");d.title=cd.n+" · "+kindLabel(k)+"\n"+cardContractType(k);d.dataset.cardKey=k;d.draggable=!!open;d.innerHTML=`<div class="cc-head"><span class="cc-rar var-badge var-${v}">${open?variantBadge(v):(LANG==="tr"?"KİLİTLİ":"LOCKED")}</span><span class="cc-kind">${open?kindLabel(k):""}</span></div><div class="cc-art">${cardStampMarkup(open?k:"__locked__",open?cd.i:LOCK_SVG,"stamp-collection")}</div><div class="cc-body"><div class="cc-name">${open?cd.n:(x.ui.lockedCard)}</div><div class="cc-desc">${desc}</div>${preview}</div><div class="cc-foot"><span class="cc-state">${equipLine}</span>${active?'<span class="cc-dot"></span>':""}</div>`;if(open)d.onclick=()=>window.CopaMobileShell?CopaMobileShell.openCard(k,[...cards]):toggleCardActive(k);grid.appendChild(d);});const cc=$("collCount");if(cc)cc.textContent=" "+owned+"/"+allCardKeys().filter(k=>!isInstantCard(k)).length+(shown!==allCardKeys().length?" · "+shown:"");}
function togglePintiSavingsWidget(){window._pintiSavingsCollapsed=!window._pintiSavingsCollapsed;syncPintiSavingsWidget();}
function syncPintiSavingsWidget(){
  let el=document.getElementById("pintiSavingsWidget");
  const hub=document.getElementById("hub"),active=typeof chairman!=="undefined"&&chairman&&chairman.id==="pinti"&&typeof runEnded!=="undefined"&&!runEnded&&hub&&!hub.classList.contains("hidden");
  if(!active){if(el)el.remove();return;}
  if(!el){el=document.createElement("aside");el.id="pintiSavingsWidget";el.className="pinti-savings-widget";document.body.appendChild(el);}
  const collapsed=!!window._pintiSavingsCollapsed,savings=Math.max(0,Math.round(typeof pintiSavings==="number"?pintiSavings:0)),power=Math.floor(savings/4),cash=savings-power*4,tr=LANG==="tr";
  el.classList.toggle("is-collapsed",collapsed);
  el.innerHTML=`<button type="button" class="pinti-savings-toggle" onclick="togglePintiSavingsWidget()" aria-expanded="${collapsed?"false":"true"}" aria-label="${tr?(collapsed?"Birikimi aç":"Birikimi kapat"):(collapsed?"Open savings":"Close savings")}">${collapsed?"+":"−"}</button><div class="pinti-savings-mark" aria-hidden="true">₺</div><div class="pinti-savings-copy"><span>${tr?"PİNTİ BİRİKİMİ":"MISER SAVINGS"}</span><strong>€${savings}M</strong><small>${tr?`Final: +${power} güç · €${cash}M kalan`:`Final: +${power} power · €${cash}M remainder`}</small></div>`;
}
...
function matchAttendance(matchRound,homePower,awayPower){
  const bases=[8000,14000,22000,34000,52000,75000,90000];
  const index=Math.max(0,Math.min(6,(Number(matchRound)||1)-1));
  return Math.round(bases[index]*(.65+Math.min(.33,Math.max(0,((Number(homePower)||0)-(Number(awayPower)||0))/100)))/1000)*1000;
}
function attachMarketCardTilt(el){if(!el||el.dataset.marketTilt)return;el.dataset.marketTilt="1";const reset=()=>{el.style.setProperty("--card-rx","0deg");el.style.setProperty("--card-ry","0deg");el.style.setProperty("--card-lift","0px");};el.addEventListener("pointermove",e=>{if(e.pointerType==="touch")return;const r=el.getBoundingClientRect(),px=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width)),py=Math.max(0,Math.min(1,(e.clientY-r.top)/r.height));el.style.setProperty("--card-rx",`${(px-.5)*9}deg`);el.style.setProperty("--card-ry",`${(.5-py)*9}deg`);el.style.setProperty("--card-lift","-3px");},{passive:true});el.addEventListener("pointerleave",reset);el.addEventListener("pointercancel",reset);}
function renderRelationshipNotice(){
  const el=$("relationshipNotice");
  if(!el)return;
  const summary=window.CopaRelationships&&typeof CopaRelationships.summary==="function"?CopaRelationships.summary():null,decision=summary&&summary.lastDecision;
  if(!decision){el.className="relationship-notice hidden";el.innerHTML="";return;}
  const tr=LANG==="tr",esc=value=>String(value==null?"":value).replace(/[&<>"']/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[ch]));
  const choiceLabel={support:tr?"Özel konuşma":"Private talk",bench:tr?"Rol netleştirildi":"Role clarified",compromise:tr?"Orta yol":"Compromise",grant:tr?"İzin verildi":"Leave granted",deny:tr?"Takım öne kondu":"Team first"}[decision.choice]||decision.choice;
  const groupLabels={captain:tr?"Kaptan çevresi":"Captain group",youth:tr?"Gençler":"Youth",stars:tr?"Yıldızlar":"Stars",local:tr?"Yerli çekirdek":"Local core"};
  const groupNames=(Array.isArray(decision.groups)?decision.groups:[]).map(id=>groupLabels[id]).filter(Boolean);
  const groupText=groupNames.length?(Number(decision.groupDelta)||0)<0?`${groupNames.join(" · ")} · ${tr?"moral gerildi":"mood strained"}`:`${groupNames.join(" · ")} · ${tr?"moral desteklendi":"mood supported"}`:"";
  const power=Number(decision.power)||0,tone=power<0?"is-risk":power>0?"is-positive":"is-neutral";
  const powerText=power>0?`+${power}`:power<0?`−${Math.abs(power)}`:"±0";
  el.className=`relationship-notice ${tone}`;
  el.innerHTML=`<div class="relationship-notice-icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l7 3v5.2c0 4.3-3 7.7-7 9.8c-4-2.1-7-5.5-7-9.8V6l7-3z"/><path d="M8.5 12l2.1 2.1l4.9-5"/></svg></div><div class="relationship-notice-copy"><span>${tr?"OYUNCU GÜVENİ":"PLAYER CONFIDENCE"} · ${esc(choiceLabel)}</span><b>${esc(decision.player)}</b><small>${esc(decision.story||"")}${groupText?` · ${esc(groupText)}`:""}</small></div><div class="relationship-notice-metrics"><span><small>${tr?"BAĞ":"BOND"}</small><b>${Number(decision.bondBefore)||0} → ${Number(decision.bondAfter)||0}</b></span><span><small>${tr?"BU MAÇ":"THIS MATCH"}</small><b>${powerText} ${tr?"GÜÇ":"POWER"}</b></span></div>`;
}
function renderHub(){if(typeof _currentCaptainPlayer==="function")_currentCaptainPlayer();try{if(typeof _saveState==="function")_saveState();}catch(e){}const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  const yc2=$("youCrest");if(yc2){const own=window.CopaClubVisuals&&window.CopaClubVisuals.crestFor(teamName||"XI"),lbl=own?own.code:(teamName||"XI").replace(/\s+/g,"").slice(0,2).toUpperCase(),cosmetics=window.CopaMeta&&typeof CopaMeta.activeCosmetics==="function"?CopaMeta.activeCosmetics():{},museumCrest=cosmetics.crest==="crest_boardroom";yc2.innerHTML=museumCrest?mkShield("#101D28","#D6A21F","#F3F5F4",lbl):mkShield(kit.bg,kit.sec||kit.fg,kit.fg,lbl);}const oc2=$("oppCrest");if(oc2&&opponent){const _lm={ENG:typeof CLUB_LOGOS_EN!=="undefined"?CLUB_LOGOS_EN:{},ES:typeof CLUB_LOGOS_ES!=="undefined"?CLUB_LOGOS_ES:{},IT:typeof CLUB_LOGOS_IT!=="undefined"?CLUB_LOGOS_IT:{},DE:typeof CLUB_LOGOS_DE!=="undefined"?CLUB_LOGOS_DE:{},JP:typeof CLUB_LOGOS_JP!=="undefined"?CLUB_LOGOS_JP:{}},_logoMap=_lm[selectedCountry]||(typeof CLUB_LOGOS!=="undefined"?CLUB_LOGOS:{}),logo=!window.COPA_IS_NATIVE&&_logoMap[opponent.name];if(logo){oc2.innerHTML=`<img src="${logo}" class="club-logo" alt="${opponent.name}">`;}else{const crest=window.CopaClubVisuals&&window.CopaClubVisuals.crestFor(opponent.name),lbl=crest?crest.code:opponent.name.replace(/\s+/g,"").slice(0,2).toUpperCase(),colors=crest?crest.colors:["#DA3D2E","#420102"];oc2.innerHTML=mkShield(colors[0],colors[1],"#F3F5F4",lbl);}}}
  const _pwCol=v=>typeof ovTextCol==="function"?ovTextCol(v):"var(--status-info-text)";
  ...
  {const used=shopRerolledThisTurn>=1,reward=used&&canOfferRewardedMarketReroll(),busy=!!shopRewardInFlight,disabled=used&&!reward&&!busy;const title=busy?LT("Reklam hazırlanıyor…","Loading ad…","Cargando anuncio…","Anzeige wird geladen…","Caricamento annuncio…"):reward?LT("Reklam izle ve ücretsiz yenile","Watch ad for a free refresh","Ver anuncio para renovar gratis","Werbung für gratis Aktualisierung","Guarda un annuncio per aggiornare gratis"):used?LT("Yenileme kullanıldı","Refresh used","Renovación usada","Aktualisierung verbraucht","Aggiornamento usato"):LT("Ücretsiz yenile","Free refresh","Renovar gratis","Gratis aktualisieren","Aggiorna gratis");const badge=reward?"AD":used?"✓":LT("ÜCRETSİZ","FREE","GRATIS","GRATIS","GRATIS");$("youLbl").textContent=x.youLbl;$("oppLbl").textContent=x.oppLbl;$("shopLbl").innerHTML=`<span class="shop-label-wrap"><span class="shop-label-title"><svg viewBox="0 0 18 16" width="12" height="11" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="1" y="5" width="11" height="10" rx="1.5"/><rect x="4" y="2.5" width="11" height="10" rx="1.5" fill="var(--paper2)"/><rect x="6" y="0" width="11" height="10" rx="1.5" fill="var(--paper2)"/></svg>${x.shopLbl}</span><button type="button" class="shop-reroll-btn${reward?" is-rewarded":""}" onclick="event.stopPropagation();shopReroll()" title="${title}" aria-label="${title}" ${disabled||busy?"disabled":""} ${busy?"aria-busy=\"true\"":""}><svg viewBox="0 0 20 20" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M16 7a6 6 0 0 0-10.5-2.5L4 6"/><path d="M4 3v3h3"/><path d="M4 13a6 6 0 0 0 10.5 2.5L16 14"/><path d="M16 17v-3h-3"/></svg><span>${busy?"…":badge}</span></button></span>`;}$("feedHdr").innerHTML=`<span class="tclive">●</span> <span>${x.feedHdr}</span>`;$("powHdr").innerHTML=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" width="12" height="12" style="vertical-align:-.1em;margin-right:3px"><path d="M3 12h4.5l1.5 -6l4 12l2 -9l1.5 3h4.5"/></svg>${x.powHdr}`;$("powV").textContent=sp.power;powerHist[round-1]=sp.power;
  {const isGhost=!!(opponent&&opponent.ghost),nm=$("oppNm"),metaId="ghostMeta";let meta=$(metaId);if(!meta&&nm&&nm.parentElement){meta=document.createElement("div");meta.id=metaId;meta.className="ghost-meta";nm.parentElement.appendChild(meta);}if(meta){meta.classList.toggle("hidden",!isGhost);meta.textContent="";if(isGhost){const gm=opponent.ghostMeta||{},icon=document.createElement("span"),copy=document.createElement("span"),report=document.createElement("button");icon.setAttribute("aria-hidden","true");icon.innerHTML=window.GhostClubs?window.GhostClubs.ghostIcon():"";copy.textContent=[LANG==="tr"?"Ger\u00e7ek oyuncu turu":"Real player run",gm.formation,gm.chairman,gm.country,gm.publicId].filter(Boolean).join(" \u00b7 ");report.type="button";report.className="ghost-report-btn";report.textContent=LANG==="tr"?"BU KULÜBÜ BİLDİR":"REPORT CLUB";report.onclick=()=>{window.GhostClubs.reportGhost(gm.publicId).then(result=>{if(result&&result.hidden){if(typeof showToast==="function")showToast(LANG==="tr"?"Kulüp bildirildi ve gizlendi.":"Club reported and hidden.");if(meta)meta.classList.add("hidden");}});};meta.append(icon,copy,report);}}if(isGhost){$("oppLbl").textContent=LANG==="tr"?"HAYALET KUL\u00dcP":"GHOST CLUB";}}
  ...
  {const tt=$("trustTile");if(tt){const tv=typeof chairTrust!=="undefined"?chairTrust:3;const tbg=tv>=3?"var(--status-success)":tv>=2?"var(--status-warning)":tv>=1?"var(--status-risk)":"var(--status-critical)";const tfg="var(--color-ink)";const tdots="●".repeat(tv)+"○".repeat(Math.max(0,3-tv));tt.classList.add("context-metric");tt.dataset.metricTone=tv>=3?"good":tv>=2?"average":tv>=1?"weak":"worst";tt.style.setProperty("--metric-accent",tbg);tt.style.background="";const trustV=$("trustV"),trustHint=$("trustHint");if(trustV){trustV.textContent=tdots;trustV.style.color=tv>=3?"var(--status-positive-text)":tv>=2?"var(--status-warning-text)":"var(--status-negative-text)";trustV.style.fontSize="14px";trustV.style.letterSpacing="4px";}const th=$("trustHdr");if(th){th.textContent=LT("BAŞKAN GÜVENİ","CHAIRMAN TRUST","CONFIANZA DEL PRESIDENTE","PRÄSIDENTENVERTRAUEN","FIDUCIA DEL PRESIDENTE");th.style.color=tfg;}if(trustHint){trustHint.textContent=tv>=3?(LANG==="tr"?"güvende":"secure"):tv>=2?(LANG==="tr"?"dengede":"steady"):tv>=1?(LANG==="tr"?"kırılgan":"fragile"):(LANG==="tr"?"tehlikede":"at risk");trustHint.style.color=tfg;}}}
  ...
  {const pv=sp.power,oppPv=opponent?opponent.power:0;const pt=$("powTile");if(pt){
    ...
    const pfg=typeof ovTextCol==="function"?ovTextCol(pv):"var(--color-ink)";
    pt.classList.add("context-metric");
    pt.dataset.metricTone=pv>=90?"elite":pv>=80?"good":pv>=70?"average":pv>=60?"weak":"worst";
    ...
    const _oppBarCol=oppPv>=pv?"rgba(66,1,2,.45)":"rgba(10,17,24,.18)";
    const _youBarCol=pfg==="#0A1118"?"rgba(10,17,24,.3)":"rgba(255,255,255,.45)";
    const _bonuses=[{l:LANG==="tr"?"Yıldız":"Star",v:Math.round(sp.starImpact||0)},{l:LANG==="tr"?"Kart":"Card",v:Math.round(sp.cardBonus||0)},{l:LANG==="tr"?"Kiralık":"Loan",v:Math.round(sp.loanBonus||0)},{l:LANG==="tr"?"Stil":"Style",v:Math.round(sp.styleBonus||0)},{l:"Kimya",v:Math.round(sp.chem||0)},{l:LANG==="tr"?"Risk":"Risk",v:Math.round(sp.risk||0)}];
  ...
  cards.forEach(k=>{const d=document.createElement("div");const v=variantOf(k);d.className="card v"+v+" cat-"+(CATMAP[k]||"gorev");const cd=x.cards[k],cv=cardVisiblePower(k,s,round),stance=variantBadge(v);d.innerHTML=`<span class="card-ico">${cardStampMarkup(k,cd.i,"stamp-mini")}</span><b>${cd.n}</b><span class="tier">${kindLabel(k)}</span> <span class="v">${cv>=0?"+":""}${cv}</span><span class="copy active-stance stance-${v}">${stance}</span>`;cr.appendChild(d);});
  if(window.CopaCardSynergy){const syn=CopaCardSynergy.calculate(cards);if(syn.power!==0){const chip=document.createElement("div");chip.className="card style synergy-chip";chip.innerHTML=`<b>${LANG==="tr"?"SİNERJİ":"SYNERGY"}</b><span class="v">${syn.power>=0?"+":""}${syn.power}</span>`;cr.appendChild(chip);}}
  {const fp=$("finalPills");if(fp){const active=cards.filter(k=>cardVisiblePower(k,s,round)!==0);if(active.length){fp.innerHTML=active.slice(0,5).map(k=>{const cd=x.cards[k],v=cardVisiblePower(k,s,round),cls=v>0?"fp-bonus":"fp-risk";return `<span class="fpill ${cls}" onclick="showCardPopup('${k}')"><span class="fpico">${cardStampMarkup(k,cd.i,"stamp-mini")}</span> ${cd.n} <b>${v>=0?"+":""}${v}</b></span>`;}).join("")+(active.length>5?`<span class="fpill fp-none">+${active.length-5}</span>`:"");}else fp.innerHTML="";}}
  ...
  const sc=$("shopcards");sc.innerHTML="";
  {
    ...
    if(!marketHead){marketHead=document.createElement("section");marketHead.id="marketDecisionHeader";marketHead.className="market-decision-head";sc.parentElement.insertBefore(marketHead,sc);}
    const debt=typeof chairmanSackLimit==="function"?chairmanSackLimit():DEBT_LIMIT;
    const identity=window.CopaMarketIdentity?CopaMarketIdentity.label(round,LANG):{id:"normal",tone:"normal",title:LANG==="tr"?"NORMAL PİYASA":"NORMAL MARKET",effect:LANG==="tr"?"Standart fiyatlar geçerli.":"Standard prices apply."};
    const safeSpan=Math.max(1,0-debt),cashRatio=Math.max(0,Math.min(1,(budget-debt)/safeSpan)),cashState=budget<=debt?"critical":budget<debt*.55?"risk":budget<0?"watch":"good";
    marketHead.dataset.marketTone=identity.tone;marketHead.dataset.cashTone=cashState;
    marketHead.innerHTML=`<div class="market-condition"><span aria-hidden="true">◆</span><div><b>${identity.title}</b><small>${identity.effect}</small></div></div><div class="market-cash-panel"><div class="market-money"><span><small>${LANG==="tr"?"MEVCUT KASA":"CURRENT CASH"}</small><b>${runMoney(budget)}</b></span><span><small>${LANG==="tr"?"BORÇ LİMİTİ":"DEBT LIMIT"}</small><b>${runMoney(debt)}</b></span></div><div class="market-cash-track" role="meter" aria-label="${LANG==="tr"?"Kasa güvenli mesafesi":"Cash safety margin"}" aria-valuemin="${debt}" aria-valuemax="0" aria-valuenow="${budget}"><i style="--cash-ratio:${cashRatio}"></i><em style="--cash-ratio:${cashRatio}"></em></div><div class="market-cash-labels"><span>${LANG==="tr"?"LİMİT":"LIMIT"}</span><span>€0M</span></div></div>`;
  }
  if(typeof lotteryCouponAmount!=="undefined"&&typeof lotteryCouponTurns!=="undefined"&&lotteryCouponAmount>0&&lotteryCouponTurns>0){
    ...
    coupon.className="lottery-coupon-banner";
  ...
  shopOffers.slice(0,3).forEach(k=>{
    ...
    const d=document.createElement("div");
     d.className="cardtile market-card sv-"+sv+" cat-"+cat+(cant?" cant":"")+(!tradeReady?" trade-missing":"");
    d.dataset.cardKey=k;
    d.title=cd.n+"\n"+cardContractType(k);
    ...
    const _powerDelta=(pv&&Number(pv.delta))||0;
    const _impactScope=instantShopPowerDelta(k,sv)!==null?(LANG==="tr"?"BU MAÇ":"THIS MATCH"):(LANG==="tr"?"İLK 11":"STARTING XI");
    const _effectSummary=window.CopaCardEffectSummary?CopaCardEffectSummary.render(CopaCardEffectSummary.summary(k,sv,{power:_powerDelta,price:pr})):_costList;
    const _detailCostList=_costLines.length?`<div class="ct-detail-costs">${_costList}</div>`:"";
     const priceMarkup=pr<=0?priceLabel:`<span class="market-card-currency">€</span>${pr}M`;
     d.innerHTML=`<div class="market-card-edge" aria-hidden="true"></div><div class="market-card-face"><div class="market-card-ink" aria-hidden="true"></div><div class="ct-head market-card-top"><span class="ct-price ct-head-price ${pr<=0?"ct-price-free":""}">${priceMarkup}</span></div><div class="ct-titlegroup market-card-stamp-zone"><span class="ct-art market-card-stamp" aria-hidden="true">${cardStampMarkup(k,cd.i,sv===1?"stamp-tile stamp-dark stamp-market-hero":"stamp-tile stamp-base stamp-market-hero")}</span></div><div class="market-card-impact"><span>${_impactScope} <b class="${_powerDelta>=0?"is-positive":"is-negative"}">${_powerDelta>=0?"+":""}${_powerDelta}</b></span></div><div class="ct-name market-card-title">${cd.n}</div><div class="ct-desc market-card-desc">${desc}</div><div class="ct-foot market-card-bottom"><span class="ct-contract market-card-mode">${cardContractType(k)}</span><button type="button" class="ct-detail" onclick="event.stopPropagation();if(window.CopaAnalytics)CopaAnalytics.track('card_effect_summary_viewed',{detail:'market',card_kind:'${analyticsCardKind(k)}'});CopaMobileShell.openMarketCard(this)">${LT("ÖZET","SUMMARY","RESUMEN","ÜBERSICHT","SINTESI")}</button></div></div><div class="ct-detail-data" hidden>${_effectSummary}${_detailCostList}${_syn?`<div class="synergy-preview">${LANG==="tr"?_syn.tr:_syn.en}</div>`:""}</div><div class="insufficient-pop" aria-hidden="true">${LANG==="tr"?"Kasa yetersiz":"Insufficient funds"}</div>`;
     attachMarketCardTilt(d);
  ...
  setBudget();_fixHubVisibleText();renderFixtures();renderFeed();_renderFreeAgents();_initHubDragDrop();_initBenchPanelDrag();if(window.CopaTournamentRuntime)window.CopaTournamentRuntime.renderHub();if(window.CopaMobileShell)window.CopaMobileShell.enhanceHub();}
...
function _hubRestoreDragStyles(style,snapshot){
...
function _hubUnlockDragScroll(){
  ...
  _hubDragScrollLock=null;
  if(_hubDragPreview){_hubDragPreview.remove();_hubDragPreview=null;}
}
function _hubCreateDragPreview(source){
  if(_hubDragPreview)_hubDragPreview.remove();
  const preview=document.createElement("div");
  preview.className="hub-player-drag-preview";
  const power=(source.querySelector(".rov,.bench-power")||{}).textContent||"";
  const pos=(source.querySelector(".rpos,.bench-pos")||{}).textContent||"";
  const name=(source.querySelector(".rname,.bench-name")||{}).textContent||"";
  const rating=document.createElement("b"),role=document.createElement("small"),label=document.createElement("span");
  rating.textContent=power.trim();role.textContent=pos.trim();label.textContent=name.trim();
  preview.append(rating,role,label);document.body.appendChild(preview);_hubDragPreview=preview;
  return preview;
}
function _hubBeginNativeDrag(event,source){
  ...
  if(event.dataTransfer){
    ...
    try{event.dataTransfer.setData("text/plain","copa-lineup-player");}catch(_){}
    const preview=_hubCreateDragPreview(source);
    try{event.dataTransfer.setDragImage(preview,preview.offsetWidth/2,preview.offsetHeight/2);}catch(_){}
  }
}
function _hubFinishNativeDrag(source){
  if(source)source.classList.remove("hub-drag-source");
  _hubUnlockDragScroll();
}
...
function _initHubDragDrop(){
  ...
  slots.forEach((_,i)=>{
    ...
    el.ondrop=e=>{e.preventDefault();_clearOver();if(!_src||_src.idx===i){_hubFinishNativeDrag();return;}
      ...
      _src=null;_hubFinishNativeDrag();};
  });
  ...
  function _addTouchDrag(el,src){
    ...
    el.addEventListener("touchstart",e=>{
      ...
      _tTimer=setTimeout(()=>{
        ...
        _tGhost=_hubCreateDragPreview(el);
        _tGhost.classList.add("touch-drag-ghost");
        _tGhost.style.left=(t.x-_tGhost.offsetWidth/2)+"px";_tGhost.style.top=(t.y-_tGhost.offsetHeight/2)+"px";
        el.classList.add("hub-drag-source");
      },320);
    },{passive:true});
    el.addEventListener("touchmove",e=>{
      const t=e.touches[0];
      if(!_tSrc){
        if(_tStart&&t&&Math.hypot(t.clientX-_tStart.x,t.clientY-_tStart.y)>9){clearTimeout(_tTimer);_tStart=null;}
        return;
      }
      e.preventDefault();
...
function showCardPopup(k){const x=L(),cd=x.cards[k];if(!cd)return;const v=cardEff(k,picksBySlot.filter(Boolean),round);const kin=kindLabel(k);const tr=LANG==="tr";
  ...
  showModal(`<div style="padding:14px 16px"><div class="card-popup-ico">${cardStampMarkup(k,cd.i,"stamp-popup")}</div><div style="font-family:var(--mono);font-weight:700;font-size:13px;text-align:center">${cd.n}</div><div style="font-family:var(--mono);font-size:9px;color:var(--ink2);text-align:center;letter-spacing:1px;margin:2px 0">${kin}</div><hr style="border:none;border-top:1px solid var(--line);margin:8px 0"><div class="card-desc-rich" style="font-size:11px;line-height:1.55;color:var(--ink)">${formatCardDesc(variantDesc(cd.d,variantOf(k)||0))}</div>${effRow}<div style="margin-top:12px"><button class="btn btn-ghost" onclick="closeModal()" style="width:100%">${x.presClose||"Kapat"}</button></div></div>`);}
function canOfferRewardedMarketReroll(){return !!(window.CopaAds&&CopaAds.canReward("showRewardedMarket")&&rewardedMarketRerollsEarned<MAX_REWARDED_MARKET_REROLLS);}
async function requestRewardedMarketReroll(){if(shopRewardInFlight||!canOfferRewardedMarketReroll())return;shopRewardInFlight=true;renderHub();let result={earned:false,reason:"ad_error"};try{result=await window.CopaAds.showRewardedMarket(chairmanEventRunId);}catch(_){ }shopRewardInFlight=false;if(result&&result.earned){rewardedMarketRerollsEarned=Math.min(MAX_REWARDED_MARKET_REROLLS,Math.max(rewardedMarketRerollsEarned+1,Number(result.earnedCount)||0));newShopOffers();_saveState();showToast(LT("Kart pazarı ücretsiz yenilendi.","Card market refreshed for free.","Mercado de cartas renovado gratis.","Kartenmarkt gratis aktualisiert.","Mercato carte aggiornato gratis."),{type:"info"});sfxTick();renderHub();return;}showToast(result&&["not_ready","not_available"].includes(result.reason)?LT("Reklam henüz hazır değil. Biraz sonra tekrar dene.","The ad is not ready. Try again shortly.","El anuncio aún no está listo. Inténtalo pronto.","Die Anzeige ist noch nicht bereit. Versuche es gleich erneut.","L'annuncio non è pronto. Riprova tra poco."):LT("Reklam tamamlanmadı. Pazar yenilenmedi.","The ad was not completed. Market unchanged.","El anuncio no se completó. Mercado sin cambios.","Die Anzeige wurde nicht abgeschlossen. Markt unverändert.","L'annuncio non è stato completato. Mercato invariato."));renderHub();}
function shopReroll(){if(shopRerolledThisTurn>=1){if(canOfferRewardedMarketReroll())return requestRewardedMarketReroll();showModal(`<div class="bulletin"><div class="bhead"><span>${LT("KART PAZARI","CARD MARKET","MERCADO DE CARTAS","KARTENMARKT","MERCATO CARTE")}</span></div><div class="bbody">${LT("Bu tur için tüm yenileme haklarını kullandın.","You used all refreshes for this run.","Has usado todas las renovaciones de esta partida.","Alle Aktualisierungen für diesen Lauf sind verbraucht.","Hai usato tutti gli aggiornamenti per questa partita.")}</div><div class="bact"><button class="btn btn-ghost" onclick="closeModal()">${LT("TAMAM","OK","OK","OK","OK")}</button></div></div>`);return;}shopRerolledThisTurn++;newShopOffers();_saveState();renderHub();sfxTick();}
function buyCard(k,overridePrice){
  if(buyCard.locked||window.CopaRunState&&window.CopaRunState.phase!=="hub")return;
  if(!CARDDEFS[k]||!Array.isArray(shopOffers)||!shopOffers.includes(k))return;
  ...
  const usedLotteryAmount=usedLotteryCoupon?lotteryCouponAmount:0;
  spend(pr,"spent");
  if(typeof chairman!=="undefined"&&chairman&&chairman.id==="pinti"&&(CARDDEFS[k]&&CARDDEFS[k].price||0)>=10&&typeof trackChairmanMetric==="function")trackChairmanMetric("premiumCardsPurchased",1);
  if(typeof chairmanReactToSpend==="function")chairmanReactToSpend(pr,"card",{card:k,variant:sv});
  recordDebt();
  cardsBoughtThisTurn++;
  if(usedLotteryCoupon){
    lotteryCouponAmount=0;lotteryCouponTurns=0;
    pushFeed("🎟️ "+(LANG==="tr"?"Piyango kuponu kullanıldı: -€":"Lottery coupon used: -€")+usedLotteryAmount+"M","buy");
  }
