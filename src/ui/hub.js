...
function renderHub(){if(typeof _currentCaptainPlayer==="function")_currentCaptainPlayer();try{if(typeof _saveState==="function")_saveState();}catch(e){}const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  {const vs=document.querySelector("#hub .vsbar");if(vs){let strip=document.getElementById("matchRiskStrip");if(!strip){strip=document.createElement("div");strip.id="matchRiskStrip";strip.className="match-risk-strip";vs.insertAdjacentElement("afterend",strip);}const warnings=typeof matchRiskWarnings==="function"?matchRiskWarnings(sp):[];strip.classList.toggle("hidden",!warnings.length);strip.innerHTML=warnings.map(item=>`<span>! ${item}</span>`).join("");}}
  ...
  {let marketHead=document.getElementById("marketDecisionHeader");if(!marketHead){marketHead=document.createElement("section");marketHead.id="marketDecisionHeader";marketHead.className="market-decision-head";sc.parentElement.insertBefore(marketHead,sc);}const spentNow=Number(typeof econStats!=="undefined"&&econStats&&econStats.spent)||0,debt=typeof chairmanSackLimit==="function"?chairmanSackLimit():DEBT_LIMIT,conditions=LANG==="tr"?["YERLİ OYUNCU HAFTASI","FORVET BOLLUĞU","DARK KART BASKISI","DENGELİ PİYASA"]:["LOCAL PLAYER WEEK","FORWARD SURPLUS","DARK CARD PRESSURE","BALANCED MARKET"],activeFilter=window._marketFilter||"impact";marketHead.innerHTML=`<div class="market-condition"><small>${LANG==="tr"?"BU TUR":"THIS ROUND"}</small><b>${conditions[(round-1)%conditions.length]}</b></div><div class="market-money"><span>${LANG==="tr"?"Kasa":"Cash"} <b>${runMoney(budget)}</b></span><span>${LANG==="tr"?"Borç limiti":"Debt limit"} <b>${runMoney(debt)}</b></span><span>${LANG==="tr"?"Harcanan":"Spent"} <b>€${spentNow}M</b></span></div><div class="market-filters">${[["impact",LANG==="tr"?"İlk 11 etkisi":"XI impact"],["position",LANG==="tr"?"Mevki":"Position"],["price",LANG==="tr"?"Fiyat":"Price"],["chem",LANG==="tr"?"Kimya":"Chemistry"]].map(([id,label])=>`<button type="button" class="${activeFilter===id?"on":""}" onclick="window._marketFilter='${id}';renderHub()">${label}</button>`).join("")}</div>`;}
  ...
  shopOffers.slice(0,2).forEach(k=>{
    ...
    d.innerHTML=`<div class="ct-head">${_darkBadge}<span class="ct-price ct-head-price ${pr<=0?"ct-price-free":""}">${priceLabel}</span></div><div class="ct-body"><div class="ct-titlegroup"><span class="ct-art" aria-hidden="true">${CARD_SVGS[k]||cd.i}</span><div class="ct-name">${cd.n}</div></div><div class="market-card-impact"><span>${LANG==="tr"?"İlk 11":"Starting XI"} <b class="${_powerDelta>=0?"is-positive":"is-negative"}">${_powerDelta>=0?"+":""}${_powerDelta}</b></span><span>${LANG==="tr"?"Kimya":"Chemistry"} <b>—</b></span><span>${LANG==="tr"?"Alım sonrası":"After purchase"} <b class="${_cashAfter<0?"is-negative":""}">${runMoney(_cashAfter)}</b></span></div><div class="ct-desc">${desc}</div><div class="ct-contract">${cardContractType(k)}</div>${_syn?`<div class="synergy-preview">${LANG==="tr"?_syn.tr:_syn.en}</div>`:""}</div><div class="ct-foot ct-foot-cost">${_costList}</div><div class="insufficient-pop" aria-hidden="true">${LANG==="tr"?"Kasa yetersiz":"Insufficient funds"}</div>`;
