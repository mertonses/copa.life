...
function renderHub(){try{if(typeof _saveState==="function")_saveState();}catch(e){}const x=L(),sp=squadPower(round),s=picksBySlot.filter(Boolean);
  ...
  /* Kaptan chip */
  if(captainIdx>=0&&picksBySlot[captainIdx]){const cp=picksBySlot[captainIdx];const injured=cp.injured;const isLider=cp.trait==="lider";const isWonder=cp.trait==="wonderkid";const capBonusVal=injured?-3:(isLider?3:isWonder?2:cp.age>=32?2:1);const dc=document.createElement("div");dc.className="card style cap-chip"+(injured?" cap-inj":"")+(isLider?" cap-lider":"");if(isLider&&!injured)dc.style.cssText="border-color:#a78bfa;background:rgba(167,139,250,.15)";dc.innerHTML=`${isLider&&!injured?'<svg viewBox="0 0 12 9" width="11" height="8" fill="#a78bfa" style="margin-right:3px"><path d="M1 8L11 8L10 3.5L7 6.5L6 1.5L3 6.5L2 3.5Z"/></svg>':`<svg viewBox="0 0 12 9" width="11" height="8" fill="currentColor" style="margin-right:3px"><path d="M1 8L11 8L10 3.5L7 6.5L6 1.5L3 6.5L2 3.5Z"/></svg>`}<b>${surOf(cp)}</b><span class="tier">${injured?(LANG==="tr"?"SAKAT KAPTAN":"INJ. CAPTAIN"):isLider?(LANG==="tr"?"LİDER KAPTAN":"LEADER CAPT."):(LANG==="tr"?"KAPTAN":"CAPTAIN")}</span><span class="v" style="${isLider&&!injured?"color:#a78bfa":""}">${capBonusVal>=0?"+":""}${capBonusVal}</span>`;cr.appendChild(dc);}
  /* Kiralık chip */
