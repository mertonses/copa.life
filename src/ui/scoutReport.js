...
function reportHtml(model,labels){
  ...
  const rows=[],used=new Set(),add=function(label,player,value){if(!player||used.has(player.name))return;used.add(player.name);rows.push(keyRow(label,player,value));};
  add(labels.highest,model.highest,model.highest&&model.highest.scoutOv);add(labels.playmaker,playmaker&&playmaker.item.player,playmaker&&playmaker.value);add(labels.fastest,fastest&&fastest.item.player,fastest&&fastest.value);
  return'<div class="scout-report-grid"><section class="scout-insight-card"><h3>'+esc(labels.keyPlayers)+'</h3>'+rows.slice(0,3).join("")+'</section><section class="scout-insight-card scout-watch-card"><h3>'+esc(labels.watchOut)+'</h3><ul>'+(model.watch.length?model.watch:[labels.noWarning]).slice(0,2).map(function(item){return'<li>'+esc(item)+'</li>';}).join("")+'</ul></section></div><div class="scout-matchup-hint"><span>'+esc(labels.matchup)+'</span><strong>'+esc(model.hint)+'</strong></div>';
}
