...
(function(global){
  ...
  function openViewer(value){
    ...
    global.showModal(`<div class="final-replay-viewer" id="finalReplayViewer" role="region" aria-label="${label}" aria-describedby="finalReplaySummary"><header><div><span>${tr?"GERÇEK MAÇ TEKRARI":"MATCH REPLAY"}</span><b>${teams[0]} · ${teams[1]}</b></div><button type="button" class="btn btn-ghost final-replay-icon final-replay-close" onclick="closeFinalMatchReplay()" aria-label="${tr?"Tekrarı kapat":"Close replay"}">×</button></header><p id="finalReplaySummary" class="final-replay-sr-summary">${summary(recordData)}</p><div class="final-replay-score"><span id="finalReplayMinute">0'</span><b id="finalReplayScore">0 – 0</b><span id="finalReplayCounter">1 / ${events.length}</span></div><div class="final-replay-pitch" aria-hidden="true"><i class="final-replay-half"></i><i class="final-replay-circle"></i><i id="finalReplayMarker" class="final-replay-marker"></i></div><div class="final-replay-current" id="finalReplayEvent" role="status" aria-live="polite"></div><input id="finalReplayRange" class="final-replay-range" type="range" min="0" max="${events.length-1}" value="0" aria-label="${tr?"Olay zaman çizgisi":"Event timeline"}" oninput="seekFinalMatchReplay(this.value)"><div class="final-replay-controls"><button type="button" class="btn btn-ghost final-replay-icon final-replay-prev" onclick="stepFinalMatchReplay(-1)" aria-label="${tr?"Önceki olay":"Previous event"}">←</button><button type="button" class="btn btn-primary final-replay-play" id="finalReplayPlay" aria-pressed="false" onclick="toggleFinalMatchReplay()">${tr?"OYNAT":"PLAY"}</button><button type="button" class="btn btn-ghost final-replay-icon final-replay-next" onclick="stepFinalMatchReplay(1)" aria-label="${tr?"Sonraki olay":"Next event"}">→</button><button type="button" class="btn btn-ghost final-replay-speak" onclick="speakFinalMatchReplaySummary()">${tr?"SESLİ ÖZET":"AUDIO SUMMARY"}</button></div><small>${tr?"Salt okunur · ← → olaylar · Boşluk oynat/duraklat":"Read only · ← → events · Space plays/pauses"}</small></div>`,{label,bare:true,dismissOnOverlay:true});
    renderFrame();
