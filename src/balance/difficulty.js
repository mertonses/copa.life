...
function applyRiskDraftCarryovers(){
  ...
  if(draftFatigueTurns>0){
    draftFatigueTurns--;
    riskPowerMod-=2;
    pushFeed("⚡ "+(tr?"Ciğerleri Yak yorgunluğu: -2 güç":"Burn the Lungs fatigue: -2 power"),"lose");
  }
  if(draftDebtTurns>0){
    draftDebtTurns--;
    riskPowerMod-=2;
...
function showDraftEvent(){
 ...
 const opts=[
  {i:_bolt,c:"#f59e0b",n:tr?"Ciğerleri Yak":"Burn the Lungs",badge:tr?"RİSKLİ":"RISKY",lines:[[gain,tr?"Bu maç +6 güç":"This match +6 power","risk-offer-gain"],[cost,tr?"-€8M, sonraki maç -2 güç":"-€8M, next match -2 power","risk-offer-cost"]],go:()=>{riskPowerMod+=6;spend(8,"spent");draftFatigueTurns=Math.max(draftFatigueTurns,1);pushFeed("⚡ "+(tr?"Ciğerleri Yak: +6 güç, sonraki maç -2":"Burn the Lungs: +6 power, next match -2"),"pres");}},
  {i:_case,c:"#16a34a",n:tr?"Şüpheli Sponsor":"Shady Sponsor",badge:tr?"KUMAR":"GAMBLE",lines:[[gain,tr?"+€16M hemen":"Immediate +€16M","risk-offer-gain"],[risk,tr?"%30 ihtimal -€12M geri ödeme":"30% chance -€12M clawback","risk-offer-risk"]],go:()=>{earn(16,"earned");let msg="💼 "+(tr?"Şüpheli Sponsor: +€16M":"Shady Sponsor: +€16M");if(rand()<0.30){spend(12,"spent");msg+=(tr?", geri ödeme -€12M":", clawback -€12M");}pushFeed(msg,"buy");}},
  ...
  {i:_band,c:"#dc2626",n:tr?"Bileğine Bas":"Play Through Pain",badge:tr?"SAKATLIK":"INJURY",lines:[[gain,tr?"Bu maç +5 güç":"This match +5 power","risk-offer-gain"],[risk,tr?"Maç sonu %35 sakatlık riski":"35% post-match injury risk","risk-offer-risk"]],go:()=>{riskPowerMod+=5;draftPendingInjury=1;pushFeed("🩹 "+(tr?"Bileğine Bas: +5 güç, maç sonu risk":"Play Through Pain: +5 power, post-match risk"),"pres");}},
