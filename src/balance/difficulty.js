...
function showDraftEvent(scheduledRound){
 ...
 const opts=[
  ...
  {id:"fortune",i:_case,c:"var(--risk-fortune)",c2:"var(--risk-fortune-highlight)",n:tr?"Kör Talih":"Blind Fortune",badge:tr?"KUMAR":"GAMBLE",lines:[[gain,tr?"+€10M hemen":"Immediate +€10M","risk-offer-gain"],[risk,tr?"%40 ihtimal -€20M geri ödeme":"40% chance -€20M clawback","risk-offer-risk"]],go:()=>{earn(10,"earned");let failed=false,msg="💼 "+(tr?"Kör Talih: +€10M":"Blind Fortune: +€10M");if(rand()<0.40){failed=true;spend(20,"spent");msg+=(tr?", geri ödeme -€20M":", clawback -€20M");}if(typeof trackRiskOfferOutcome==="function")trackRiskOfferOutcome("fortune",{failed,cashDelta:failed?-10:10});pushFeed(msg,"buy");}},
 ...
 if(typeof trackRiskOffersShown==="function")trackRiskOffersShown(opts.map(o=>o.id),targetRound);
 _persistDraftEventState();
 return true;
}
function pickDraftEvent(i){const o=i<0?(window._draftEvents&&window._draftEvents.find(x=>x.skip)):(window._draftEvents&&window._draftEvents[i]);if(!o)return;closeModal();if(typeof trackRiskOfferChosen==="function")trackRiskOfferChosen(o.id,round);o.go();sfxStamp();setBudget();renderHub();showToast(o.n,{type:o.skip?"default":"default"});}
