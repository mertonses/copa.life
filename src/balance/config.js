...
var BUDGET=30,DEBT_LIMIT=-28,MAX_CARD_COPIES=4,FINAL_DEBT_CAP=18;
...
function _randInjLevel(){const r=rand();return r<0.50?1:r<0.80?2:3;}
function applyRandomInjury(chance){if(injuredIdx>=0||rand()>=chance)return null;const idxs=[];picksBySlot.forEach((p,i)=>{if(p&&!p.injured)idxs.push(i);});if(!idxs.length)return null;injuredIdx=rnd(idxs);const p=picksBySlot[injuredIdx];p.injured=true;p.injuryLevel=_randInjLevel();if(econStats)econStats.injuries=(econStats.injuries||0)+1;return p;}
