...
(function(global){
  ...
  function selectDirective(id){
    ...
    state.directives.selected=state.directives.selected===key?"":key;persist();refreshProgressionView("directives");return true;
  }
  ...
  function overviewHTML(tr){
    const runs=Math.max(integer(global.metaRuns,0,1000000),state.archive.length),latest=state.archive.length?state.archive[state.archive.length-1]:null,latestResult=latest&&resultMeta(latest,tr),cash=latest?Number(latest.cash||0):0,emptyCopy=runs>0?(tr?"Ayrıntılı kayıtlar sonraki tamamlanan turundan itibaren burada görünecek.":"Detailed records will appear here after your next completed run."):(tr?"İlk tamamlanan turun burada görünecek.":"Your first completed run will appear here."),recent=state.archive.length?archiveRowsHTML(tr,3):`<div class="meta-inline-empty">${emptyCopy}</div>`;
  ...
  function financeHTML(tr){
    const runs=[...state.archive].reverse(),total=runs.reduce((sum,item)=>sum+Number(item.cash||0),0),positive=runs.filter(item=>Number(item.cash)>=0).length,average=runs.length?Math.round(total/runs.length):0;
  ...
  global.CopaMeta=Object.freeze({
    ...
    renderPanelHTML:(tab="career")=>panelHTML(normalizeDossierTab(tab),global.LANG==="tr"),
