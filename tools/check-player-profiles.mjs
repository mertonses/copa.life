...
for(const marker of ["copa-model-v1","DIMENSION_KEYS","positionFit","Karar Mekanizması","Kurtarış Etkisi","Temaslı oyunu seviyor","radarHtml","data-profile-retry","clubLogoFor","genericCrestFor","countryCode","NATIONAL_TEAM_LEVELS","A Milli","_normalizeForTest","_renderForTest"])if(!profileUi.includes(marker))throw new Error("Profile UI marker missing: "+marker);
...
const html=renderProfile(outfield);for(const marker of ["player-profile-radar-area","player-profile-fit","GÜÇLÜ YÖNLER"])if(!html.includes(marker))throw new Error("Profile render missing: "+marker);for(const marker of ["player-profile-stat","data-profile-attributes",'aria-valuemax="20"',"Copa Life oyun modeli · 0–100"])if(html.includes(marker))throw new Error("Raw attribute UI rendered: "+marker);
