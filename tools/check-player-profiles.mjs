...
for(const marker of ["copa-model-v1","DIMENSION_KEYS","positionFit","Baskı ve Karar","Kurtarış Etkisi","Temaslı oyun eğilimi","radarHtml","data-profile-retry","clubLogoFor","genericCrestFor","_normalizeForTest","_renderForTest"])if(!profileUi.includes(marker))throw new Error("Profile UI marker missing: "+marker);
...
for(const marker of [".player-profile-radar-area",".player-profile-fit",".player-profile-model-note",".is-generic-crest","prefers-reduced-motion"])if(!profileCss.includes(marker))throw new Error("Profile CSS marker missing: "+marker);
