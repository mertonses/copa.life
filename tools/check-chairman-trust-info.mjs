...
requireMatch(index,/startNum=trustNum\("1\/3","fragile"\)/,"Trust help does not explain or contextually color the starting value.");
requireMatch(index,/gain=trustNum\("\+1","positive"\),loss=trustNum\("−1","negative"\)/,"Trust help does not contextually color match-result changes.");
...
requireMatch(index,/DARK kartlar[\s\S]*?DARK cards/,"Trust help does not explain DARK-card trust loss.");
requireMatch(index,/tierTones=\["danger","fragile","steady","secure"\]/,"Trust help tiers are missing contextual tone classes.");
requireMatch(index,/trust-info-num is-\$\{tone\}/,"Trust help numeric values are missing semantic tone classes.");
...
requireMatch(css,/\.trust-info-level\.is-secure[\s\S]*?\.trust-info-level\.is-danger/,"Trust help tiers are missing contextual colors.");
requireMatch(css,/\.trust-info-num\.is-positive[\s\S]*?\.trust-info-num\.is-negative/,"Trust help numeric values are missing contextual colors.");
