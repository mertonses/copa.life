...
const powers=[64,72,80,88],sampleSize=700,report={model:core.MODEL_VERSION,sampleSize,powers:{}};
...
for(const playerPower of powers){
  let qualified=0,champion=0,points=0,groupWins=0,groupDraws=0,matches=0;
  for(let run=0;run<sampleSize;run++){
    ...
    while(state.phase!=="complete"){
      ...
      if(match.stage==="group"){
        ...
        groupWins+=gf>ga?1:0;groupDraws+=gf===ga?1:0;matches++;
      }
  ...
  report.powers[playerPower]={qualificationRate:qualified/sampleSize,championRate:champion/sampleSize,averageGroupPoints:points/sampleSize,groupWinRate:groupWins/matches,groupDrawRate:groupDraws/matches};
}
