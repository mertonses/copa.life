...
for(const relative of ["assets/clubs","assets/icons/patreon.svg","src/data/logos.js","src/runtime/productAnalytics.js","src/state/diagnostics.js","sw.js"]){if(fs.existsSync(path.join(OUT,relative)))fail(`forbidden Android artifact: ${relative}`);}
...
if(failures.length){for(const failure of failures)console.error(`[android] ${failure}`);process.exit(1);}console.log(`[android] clean package: ${walk(OUT).length} files; no real crests, embedded contact collector, raw 1–20 profile data, medical tendency, or official cup names`);
