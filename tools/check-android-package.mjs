...
const capacitorConfig=JSON.parse(fs.readFileSync(path.join(ROOT,"capacitor.config.json"),"utf8")),androidStrings=fs.readFileSync(path.join(ROOT,"android/app/src/main/res/values/strings.xml"),"utf8");
if(capacitorConfig.appName!=="Copa Life")fail("Capacitor app name must be Copa Life");
for(const marker of ['<string name="app_name">Copa Life</string>','<string name="title_activity_main">Copa Life</string>'])if(!androidStrings.includes(marker))fail(`Android device name is missing ${marker}`);
for(const marker of ["ads-mobile-sdk:1.2.1","user-messaging-platform:4.0.0","COPA_ADMOB_APP_ID","COPA_ADMOB_INTERSTITIAL_ID","COPA_ADMOB_REWARDED_ID","COPA_ADMOB_NATIVE_ID"])if(!appGradle.includes(marker))fail(`Android ad configuration is missing ${marker}`);
...
if(!index.includes("copa.life"))fail("Android game wordmark must remain copa.life");
...
for(const marker of ["src/runtime/nativeAds.js","src/runtime/adGateway.js","CopaAds.showRunEnd(window._completedGhostRunKey)"])if(!index.includes(marker))fail(`Android run-end ad hook is missing ${marker}`);
...
if(!fs.existsSync(nativeAdsRuntime))fail("Android native ad runtime is missing");else{const source=fs.readFileSync(nativeAdsRuntime,"utf8");for(const marker of ["CopaAds","showRunEnd","showArenaEnd","showRewardedReroll","showRewardedInjury","showRewardedMarket","reason:\"disabled\"","hideListPlacement","showPrivacyOptions","privacyOptionsChanged","scheduleInitialize","setTimeout(initialize,8000)"])if(!source.includes(marker))fail(`Android native ad runtime is missing ${marker}`);}
