...
for(const marker of ["MAX_REWARDED_INJURY_HEALS=2","MAX_REWARDED_MARKET_REROLLS=1"])expect(state,marker);
...
for(const marker of ["PLACEMENT_INJURY","PLACEMENT_MARKET","MAX_REWARDED_INJURY_HEALS_PER_RUN = 2","MAX_REWARDED_MARKET_REROLLS_PER_RUN = 1","onUserEarnedReward"])expect(plugin,marker);
...
if(!process.exitCode)console.log("Rewarded ads OK: draft 2/run, injury heal 2/run, market refresh 1/run; rewards require native completion.");
